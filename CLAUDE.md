...
- **Status**: 2673 tests passing; ~195 validated; 42/42 P1; 17/17 P2; 40/40 P3; 95/105 P4 (95/95 implemented; 9 permanent-n/a; 1 deferred: Banding post-alpha); Batch 0-16 + Mutate + Transform + Morph + Dungeon + Ring complete; Type consolidation COMPLETE; **ALL PRIMITIVE BATCHES COMPLETE (PB-0 through PB-21)**; PB-22 deferred cleanup DONE; W6-review 21/21 COMPLETE; 0 HIGH/MEDIUM open; **~47 LOW open** (PB-D added +1: marisi_breaker_of_the_coil stale TODO; PB-P added +5 non-blocking); W3 LOW sprint COMPLETE; PB-23–37 DONE; **PB-A/B/E/J/M/S/X/Q/Q4/N/D/P/L DONE**; PB-Q2/Q3/Q5 reserved (planned)
- **Active Plan**: **W6 Primitive + Card Authoring** — **PB-P + PB-L MERGED 2026-04-19** via ESM chain-dispatch (`8ba9c5b7`, `872ea5d2`). PB-P: `EffectAmount::PowerOfSacrificedCreature` + `AdditionalCost::Sacrifice` reshape (`{ ids, lki_powers }`) for CR 608.2b LKI-capture-by-value; 3 cards (altar_of_dementia, greater_good, lifes_legacy); 8+3 tests; hash 5→6. PB-L: collapsed-hybrid — no new `TriggerCondition` (Landfall is ability word CR 207.2c), `ETBTriggerFilter.card_type_filter` + battlefield-conversion block in `replay_harness.rs` exposed mid-task by tests (Step 0 verdict reversed from EXISTS to PARTIAL-GAP per `feedback_verify_full_chain.md`); 3 cards (khalni_heart_expedition, druid_class, omnath_locus_of_rage) + 5 TODO rewrites; 9 tests; hash 6→7; memo at `memory/primitives/pb-note-L-collapsed.md`. **Queue is now thin** — old queue (PB-R, PB-Q3, PB-T, PB-U, PB-V, PB-W, PB-Y, PB-Q2/Q5) opportunistic. Re-triage W6 backlog or pivot to LOW cleanup (~47 open) next session. W5 RETIRED.
...
- **Last Updated**: 2026-04-19 (chain-dispatch session — PB-P + PB-L shipped sequentially via ESM, 2655→2673; pattern validated: single-coordinator `/dispatch` → poll → `/collect` → `/dispatch` → poll → `/collect` with no user intervention mid-chain)
