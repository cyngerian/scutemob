...
- **Status**: ~2562 tests passing; ~195 validated; 42/42 P1; 17/17 P2; 40/40 P3; 95/105 P4 (95/95 implemented; 0 planned; 9 permanent-n/a; 1 deferred: Banding post-alpha); Batch 0-16 + Mutate + Transform + Morph + Dungeon + Ring complete; Type consolidation COMPLETE; **ALL PRIMITIVE BATCHES COMPLETE (PB-0 through PB-21)**; **PB-22 deferred cleanup ALL 7 SESSIONS DONE**; W6-review 21/21 COMPLETE; 0 HIGH/MEDIUM open; **~29 LOW open**; W3 LOW sprint COMPLETE; PB-23–37 DONE; **PB-A DONE** (play from top of library: PlayFromTopPermission, PayLifeForManaValue, 6 new cards, 4 fixes, 18 tests); **PB-B DONE** (play from graveyard: PlayFromGraveyardPermission, CastSelfFromGraveyard, ControllerGainedLifeThisTurn, 6 card fixes, 13 tests)
- **Active Plan**: **W6 Primitive + Card Authoring** — **Phase 2.5 DSL gap closure COMPLETE (PB-0–37)**. **Backfill COMPLETE**. **Wave A COMPLETE**. **Wave A+B engine reviews CLEAN**. **Wave B.5 in progress**: PB-N done (19 misc cards), **PB-G DONE** (BounceAll + 4 cards), **PB-K DONE** (land drops + Case mechanic), **PB-D DONE** (chosen creature type, 8 fixes), **PB-C DONE** (extra turns, 3 new cards, 1 fix), **PB-F DONE** (damage multiplier, 1 new card, 2 fixes), **PB-I DONE** (grant flash, 1 new card, 3 fixes), **PB-H DONE** (mass reanimate, 1 new card, 4 fixes), **PB-L DONE** (reveal/X effects, 2 new cards, 5 fixes), **PB-A DONE** (play from top of library, 6 new cards, 4 fixes), **PB-B DONE** (play from graveyard, 6 card fixes, 13 tests). **Next: HIGH batches (PB-E/J/M)** (see ops plan). W5 RETIRED. Goal: all 1,743 cards complete pre-alpha, zero TODOs.
...
- **Last Updated**: 2026-04-08 (PB-B: play from graveyard — PlayFromGraveyardPermission, CastSelfFromGraveyard, ControllerGainedLifeThisTurn, life_gained_this_turn tracker, 6 card fixes (Ancient Greenwarden, Perennial Behemoth, Wrenn+Realmbreaker, Oathsworn Vampire, Squee, Brokkos), 13 tests; ~1741 total defs; ~2562 tests, 0 clippy)
