...
- **Next dispatch**: NONE from the v4 queue — the second chain is **CLOSED at rank 21** (do NOT
  dispatch PB-DX9 / PB-DX38). LL-3 (`-257`) and LL-1 (`-255`) collected 2026-09-05. **STOP: CC-9
  hot-seat (`scutemob-245`, pod-facing) is next and awaits owner approval**; LL-2 (`-256`) after it.
  CC-5 decklists (`-241`, owner) gate LL-4 (`-258`) and CC-6/7/8. Every dispatch needs explicit
  owner approval (`feedback_queue_autonomous_chaining` RETRACTED 2026-08-01).
...
- **Rules / card / rulings search** (`mtg-rules`): by rule number, concept, or exact card name. CR text is
  authoritative; rulings can be stale. Freshness: `tools/data-freshness.py` (SessionStart hook runs `check`; OFFER `refresh`).
...
Active project agents in `.claude/agents/`. The eight dormant milestone/ability agents live in
`.claude/agents-dormant/` (README there says how to restore one; dormant skills likewise in
`.claude/skills-dormant/`):
