...
- **Status**: 2655 tests passing; ~195 validated; 42/42 P1; 17/17 P2; 40/40 P3; 95/105 P4 (95/95 implemented; 9 permanent-n/a; 1 deferred: Banding post-alpha); Batch 0-16 + Mutate + Transform + Morph + Dungeon + Ring complete; Type consolidation COMPLETE; **ALL PRIMITIVE BATCHES COMPLETE (PB-0 through PB-21)**; PB-22 deferred cleanup DONE; W6-review 21/21 COMPLETE; 0 HIGH/MEDIUM open; **~46 LOW open** (PB-D added +1: stale TODO in marisi_breaker_of_the_coil.rs); W3 LOW sprint COMPLETE; PB-23–37 DONE; **PB-A/B/E/J/M/S/X/Q/Q4/N/D DONE**; PB-Q2/Q3/Q5 reserved (planned)
- **Active Plan**: **W6 Primitive + Card Authoring** — **PB-D MERGED 2026-04-19** via ESM dispatch A/B (`72cddb62`). `TargetController::DamagedPlayer` variant + 10 dispatch sites + 6 card defs (Throat Slitter, Sigil of Sleep, Mistblade Shinobi, Alela, Nature's Will, Balefire Dragon) + 7 MANDATORY tests. Hash sentinel 4→5. **Workstream coordination migrated to ESM tasks** (per-PB task + worktree + dispatched worker); `memory/workstream-state.md` kept for history. **Next queue candidates** (oversight to pick): PB-P re-triage (real-but-narrow — `EffectAmount::PowerOf(SacrificedCreature)` LKI only; Altar of Dementia, Greater Good), PB-L rank 3 at ~3-4 calibrated yield. Old queue (PB-R, PB-Q3, PB-T, PB-U, PB-V, PB-W, PB-Y, PB-Q2/Q5) demoted — opportunistic only. See `docs/primitive-card-plan.md` Phase 1.8. W5 RETIRED.
...
- **Last Updated**: 2026-04-19 (PB-D merged via ESM A/B — agent-delegated worker won decisively vs inline; dispatch skill hardened to require TaskCreate list + specialized-agent delegation; tests 2648→2655)
