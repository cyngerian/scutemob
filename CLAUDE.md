...
- **Last Updated**: 2026-07-26 (PB-RS4 close-out) — **THE PB-OS QUEUE IS COMPLETE** (OS1..OS11 + OS4b, `scutemob-115..141`; final merge `bd220b00`). **Coverage 1,135/1,804 = 62.9%, tests 3560+, PROTOCOL 26 / HASH 63.** Highlights: 4 Complete-but-wrong made right (gain-control reversion, face-aware transform gathering); 3 known_wrong redeemed (forbidden_orchard, umezawas_jitte, delver); 2 seed premises falsified-and-reframed vs oracle (PB-OS11); DOC + audit-#2 remediation complete (docs.yaml live, skills rewired, /start-work retired). Rider-seed mini-triage DONE same day (`scutemob-142`) — filed OOS-RS-1..6, ranked queue PB-RS1..RS11 (`memory/primitives/rider-seed-triage-2026-07-19.md`). **PB-RS1..RS3 SHIPPED 2026-07-19/20** (`scutemob-143/144/145`: library top/bottom reconciliation; hybrid/Phyrexian pip payment, PROTOCOL 26→27; AtBeginningOfCombat sweep — 4 flips + helm repair total, coverage 63.1%); queue was PAUSED after R3 by user, **lifted for R4**. **PB-RS4 SHIPPED 2026-07-26** (`scutemob-146`): face-aware residuals — **OOS-RS-3 closed, and with it OOS-OS4-2 fully closed**. Both `replacement.rs` gathering sites read the active face; `deregister_face_statics` extended from 1 to all 10 registered families via a `remove_one_registration` inverse helper + a source-scan parity gate; a fourth same-root-cause deviation found in planning (CR 714.3b Saga lore sweep + `fire_saga_chapter_triggers` `ability_index` namespace, which disagreed with all 8 consumers) fixed too. 0 flips as predicted, 2 integrity repairs, 17 fail-before/pass-after probes, review 0 HIGH / 1 MEDIUM / 11 LOW all dispositioned; PROTOCOL 27 / HASH 63 unchanged. Seeds OOS-RS4-1/2/4 filed. **Next: R5** (Anim Pakal LKI counters). Detailed per-PB history: `memory/archive/claude-md-changelog-2026-07.md`.
