...
- **Status**: **2745 tests passing**; ~199 validated; 42/42 P1; 17/17 P2; 40/40 P3; 95/105 P4 (95/95 implemented; 9 permanent-n/a; 1 deferred: Banding post-alpha); Type consolidation COMPLETE; W6-review 21/21 COMPLETE; 0 HIGH/MEDIUM open; **~45 LOW open**; PB-0 through PB-37 DONE; **PB-A/B/E/J/M/S/X/Q/Q4/N/D/P/L/T/SFT/CC-{W,B,C,A}/CC-C-followup/TS/LKI-CC/CD DONE**; PB-Q2/Q3/Q5 reserved (planned); 9 stop-and-flag seeds in `memory/primitives/pb-retriage-CC.md` untouched (OOS-LKI-3/4/Power newly added; `TargetFilter.exclude_self` and Master Biomancer ETB-counter-from-source-power attractive next picks).
- **Active Plan**: **PB-CD MERGED 2026-05-13** (`scutemob-18`). Counter-doubling replacement effects (CR 122.6 / 614.1) — ships Hardened Scales, Corpsejack Menace, Conclave Mentor (replacement half only; death trigger deferred as OOS-LKI-Power seed). Engine surface: (1) `ReplacementTrigger::WouldPlaceCounters.counter_filter: Option<CounterType>` — `Some(t)` gates "+1/+1 only" cards, `None` preserves Vorinclex/Pir/Lae'zel "one or more counters" semantics; (2) `ObjectFilter::CreatureControlledBy(PlayerId)` (disc 8) — layer-resolved creature type + controller equality per CR 613.1d. HASH 15→16. Tests +11 (3 positive, 3 counter-type isolation, 2 receiver isolation, 2 stacking, 1 backward-compat). Review PASS (3 LOW: 1 CR-citation fix applied 121.6→122.6; 2 false-positives non-reproducible). Tests 2734→**2745**. Predecessor: **PB-TS + PB-LKI-CC MERGED 2026-04-30**. Next session: `TargetFilter.exclude_self` (Éomer), Master Biomancer ETB-with-counters-from-source-power (related to OOS-LKI-Power), or one of the existing OOS-LKI/TS seeds.
...
- **Last Updated**: 2026-05-13 (PB-CD merged `scutemob-18`; tests 2734→2745; HASH 15→16)
