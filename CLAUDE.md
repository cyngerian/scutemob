...
- **Status**: **2734 tests passing**; ~196 validated; 42/42 P1; 17/17 P2; 40/40 P3; 95/105 P4 (95/95 implemented; 9 permanent-n/a; 1 deferred: Banding post-alpha); Type consolidation COMPLETE; W6-review 21/21 COMPLETE; 0 HIGH/MEDIUM open; **~45 LOW open**; PB-0 through PB-37 DONE; **PB-A/B/E/J/M/S/X/Q/Q4/N/D/P/L/T/SFT/CC-{W,B,C,A}/CC-C-followup/TS/LKI-CC DONE**; PB-Q2/Q3/Q5 reserved (planned); 8 stop-and-flag seeds in `memory/primitives/pb-retriage-CC.md` untouched (OOS-LKI-3/4 newly added; counter-doubling CR 121.6 + `TargetFilter.exclude_self` attractive next picks).
- **Active Plan**: **PB-TS + PB-LKI-CC MERGED 2026-04-30** (back-to-back dispatch-collect chain). PB-TS (`scutemob-16` `68f4bfbc`): `TokenSpec.count: u32 → EffectAmount` — dynamic token count. 4 cards re-authored (Phyrexian Swarmlord, Krenko, Izoni, Chasm Skulker reverted then re-authored by PB-LKI-CC). HASH 13→14. Tests +5. Review NEEDS-FIX→PASS. PB-LKI-CC (`scutemob-17` `a2b24e42`): `EffectAmount::CounterCountAtLastKnownInformation` per CR 603.10a. LKI counter snapshot threaded through 5 `SelfLeavesBattlefield` arms (added `pre_lba_counters` to 4 `GameEvent` variants, ~35 emit sites). 2 cards (Chasm Skulker re-author, Toothy retroactive fix — was producing 0 draws on bounce/exile/destroy). HASH 14→15. Tests +9. Review PASS (E1 HIGH fixed in fix-phase). Tests 2720→**2734** (+14 overall). Next session: counter-doubling replacement (Hardened Scales et al.), `TargetFilter.exclude_self` (Éomer), or one of the OOS-LKI seeds.
...
- **Last Updated**: 2026-04-30 (PB-TS + PB-LKI-CC chain merged `scutemob-16`/`scutemob-17`; tests 2720→2734; HASH 13→15)
