...
- **Active Milestone**: M9.5 DONE — **Card Authoring Campaign ACTIVE** (plan: `memory/card-authoring/campaign-plan-2026-05-16.md` §0 recalibration 2026-07-07; clean coverage 965/1,748 = 55.2% per `tools/authoring-report.py`)
- **Tests**: **3009 passing**; build/clippy/fmt clean
...
- **Last shipped**: **PB-AC6** (`scutemob-49`, merge `0628807e`) — `TriggerCondition::{AtBeginningOfFirstMainPhase, AtBeginningOfPostcombatMain}` (generic CardDef sweeps per B9 lesson) + `WhenBecomesTarget` (fires at announcement, CR 601.2c/602.2b); `Condition::{YouAttackedThisTurn, CreatedATokenThisTurn, OpponentCastNSpells, SpellMastery, OpponentControlsMoreLandsThanYou}`; 3 new PlayerState trackers (all-player turn-boundary resets; dedicated `spells_cast_this_turn_all` — plan's reuse proposal rejected as wrong game state); hash schema →33, trackers mutation-verified in HashInto. Review 0 HIGH / 0 MEDIUM. Backfill: 6 clean + Kaito +1 + marker-correction sweep over 13 blocked cards; card review 2 HIGH + 1 MEDIUM fixed. Prior: PB-AC5 (`scutemob-47`, Warp/Transmute/Exert/pitch), PB-AC4 (`scutemob-46`), PB-AC3..AC1 (`scutemob-45..43`). Next: PB-AC7 (type-changing & ability-removal). Registry-gate debt: `scutemob-48` (invariant #9, backlog).
...
- **Last Updated**: 2026-07-09 (PB-AC6 collected — 3009 tests, clean coverage 55.2%)
