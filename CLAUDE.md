...
- **Status**: 2716 tests passing; ~196 validated; 42/42 P1; 17/17 P2; 40/40 P3; 95/105 P4 (95/95 implemented; 9 permanent-n/a; 1 deferred: Banding post-alpha); Batch 0-16 + Mutate + Transform + Morph + Dungeon + Ring complete; Type consolidation COMPLETE; **ALL PRIMITIVE BATCHES COMPLETE (PB-0 through PB-21)**; PB-22 deferred cleanup DONE; W6-review 21/21 COMPLETE; 0 HIGH/MEDIUM open; **~45 LOW open**; PB-23–37 DONE; **PB-A/B/E/J/M/S/X/Q/Q4/N/D/P/L/T/SFT/CC-W/CC-B/CC-C/CC-A DONE**; PB-Q2/Q3/Q5 reserved (planned); **PB-CC-C-followup seeded** (Layer 7c continuous CDA re-evaluation per CR 611.3a — Vishgraz + Exuberant Fuseling deferred Option B)
- **Active Plan**: **5-PB autonomous chain MERGED 2026-04-29** (`051442bd..fd6c8e6a`, pushed). Phase A re-triage: PB-SFT (`scutemob-8` `700f13c8`) verdict PROCEED — FIELD-ADDITION (planner mis-named; gap on Effect, not Cost); PB-CC (`scutemob-9` `037c3547`) verdict UMBRELLA-OF-MICRO-PRIMITIVES (4 micro-PBs). Wave 1 parallel: PB-SFT impl `scutemob-10` `da566418` (`Effect::SacrificePermanents.filter: Option<TargetFilter>`, 7+ cards re-authored: Fleshbag Marauder, Merciless Executioner, Butcher of Malakir, Dictate of Erebos, Grave Pact, Liliana DH-4, Blasphemous Edict, Roiling Regrowth) + PB-CC-W `scutemob-11` `0a46e1b4` (Mossborn Hydra Landfall stale-TODO wire-up, no engine). Wave 2 sequential: PB-CC-B `scutemob-12` `06d02990` (`TargetFilter.has_counter_type` + Armorcraft Judge), PB-CC-C `scutemob-13` `e2d75672` (`LayerModification::ModifyPower/ToughnessDynamic` engine variants — Exuberant Fuseling deferred Option B), PB-CC-A `scutemob-14` `fd6c8e6a` (`EffectAmount::PlayerCounterCount` engine variant — Vishgraz deferred Option B, same trap). Tests 2689→2716. HASH bumped 4×. ~10 cards unblocked total (matches `feedback_pb_yield_calibration.md` 2-3x discount). Workers delegated to primitive-impl-runner + primitive-impl-reviewer throughout. Next session: (a) **PB-CC-C-followup** (continuous CDA re-evaluation; unblocks Vishgraz + Exuberant Fuseling) or (b) re-triage of 9 named seeds in pb-retriage-CC.md stop-and-flag log (PB-TS token-count is biggest umbrella).
...
- **Last Updated**: 2026-04-29 (5-PB autonomous chain — PB-SFT + PB-CC umbrella; tests 2689→2716; PB-CC-C-followup seeded; 3 new ESM dispatch gotchas)
