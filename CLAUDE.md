...
- **Active Plan**: **W6 Primitive + Card Authoring** — **PB-N closed 2026-04-13** (5/33 yield = 15%, trigger-category calibration). Hash sentinel bumped 3→4 as standing rule. `combat_damage_filter` tightened to damage-only (latent bug fix bundled). Next top-3: **PB-D** (DamagedPlayer target filter, revised est 7-8) → **PB-P** (PowerOfCreature EffectAmount, revised est 7-8) → **PB-L** (Landfall trigger, revised est 3-4 — demoted by trigger calibration). Old queue (PB-R, PB-Q3, PB-T, PB-U, PB-V, PB-W, PB-Y, PB-Q2/Q5) demoted — opportunistic only. See `docs/primitive-card-plan.md` Phase 1.8 for the PB-N close summary + spawned candidates + re-discounted queue. W5 RETIRED.
...
- **Last Updated**: 2026-04-13 (PB-N closed — 5 cards shipped including Utvara Hellkite roster-recall catch; BASELINE-LKI-01 logged; trigger-PB calibration set at 15-25%; 2648 tests)
