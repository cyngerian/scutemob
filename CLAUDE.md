...
- **Status**: 2616 tests passing; ~195 validated; 42/42 P1; 17/17 P2; 40/40 P3; 95/105 P4 (95/95 implemented; 9 permanent-n/a; 1 deferred: Banding post-alpha); Batch 0-16 + Mutate + Transform + Morph + Dungeon + Ring complete; Type consolidation COMPLETE; **ALL PRIMITIVE BATCHES COMPLETE (PB-0 through PB-21)**; PB-22 deferred cleanup DONE; W6-review 21/21 COMPLETE; 0 HIGH/MEDIUM open; **~35 LOW open**; W3 LOW sprint COMPLETE; PB-23–37 DONE; **PB-A/B/E/J/M/S DONE**; **PB-X DONE** (A-42 Tier 1 micro-PB — AllCreaturesExcludingSubtype/AllCreaturesExcludingChosenSubtype EffectFilters, `LayerModification::ModifyBothDynamic` substituted at Effect::ApplyContinuousEffect per CR 608.2h, `Cost::ExileSelf` + `ActivationCost.exile_self` via existing `embedded_effect` LKI plumbing; 6 cards unblocked; 16 tests; 1 HIGH fixed — Obelisk rewritten Triggered→Replacement per CR 614.12; **close phase deferred to next session**)
- **Active Plan**: **W6 Primitive + Card Authoring** — Wave B.5 HIGH batches COMPLETE. **PB-X DONE 2026-04-11** (implement + review + fix; close deferred): 3 primitives, 6 A-42 Tier 1 cards fully authored (Crippling Fear, Eyeblight Massacre, Olivia's Wrath, Balthor the Defiled, Obelisk of Urd, City on Fire). Review C1 HIGH — Obelisk "As this enters, choose a creature type" authored as triggered ETB instead of replacement effect (CR 614.12); fixed in isolated pass before Pass 2 bundle (E1 CR citation rewrites, C2 Balthor activated e2e test, C3 Obelisk observability-window test + City on Fire tests, E2/E3 LOWs). Metallic Mimic parked as **PB-Y** (AddChosenCreatureType) — NOT scheduled ahead of PB-Q. Heritage Druid still parked on cost-framework PB. Revised slate (unchanged): PB-Q (ChooseColor) → PB-R (ExchangeZones) → PB-T+V → PB-U → PB-Y → PB-W. Reports: `memory/primitives/pb-plan-X.md`, `memory/primitives/pb-review-X.md`. W5 RETIRED.
...
- **Last Updated**: 2026-04-11 (PB-X implement + review + fix DONE; close deferred; 2616 tests; 6 cards unblocked; 1 HIGH fixed)
