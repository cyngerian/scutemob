...
- **Status**: 2627 tests passing; ~195 validated; 42/42 P1; 17/17 P2; 40/40 P3; 95/105 P4 (95/95 implemented; 9 permanent-n/a; 1 deferred: Banding post-alpha); Batch 0-16 + Mutate + Transform + Morph + Dungeon + Ring complete; Type consolidation COMPLETE; **ALL PRIMITIVE BATCHES COMPLETE (PB-0 through PB-21)**; PB-22 deferred cleanup DONE; W6-review 21/21 COMPLETE; 0 HIGH/MEDIUM open; **~35 LOW open**; W3 LOW sprint COMPLETE; PB-23–37 DONE; **PB-A/B/E/J/M/S/X DONE**; **PB-Q IMPLEMENT DONE, REVIEW PENDING** (ChooseColor — 6 cards: Caged Sun, Gauntlet of Power, Throne of Eldraine, Temple of the Dragon Queen, Utopia Sprawl; `GameObject.chosen_color`, `ReplacementModification::ChooseColor(Color)` parallel of `ChooseCreatureType`, `AddOneManaOfChosenColor` matches Mana Reflection precedent per CR 605.3 framing compromise, 2 EffectFilter variants, `Effect::AddManaOfChosenColor`, `apply_mana_production_replacements` signature refactor, hash sentinel 2→3; +11 tests)
- **Active Plan**: **W6 Primitive + Card Authoring** — **PB-Q review phase pending** (`memory/primitive-wip.md` at phase: review; runner flagged 8-item review focus list including unplanned deterministic tie-break in `ChooseColor` fallback, `apply_mana_production_replacements` refactor ripple check, Utopia Sprawl Aura+ETB ordering). After PB-Q close: **PB-Q2** (activated-time choose-color — Skrelv, Nykthos, Three Tree City, Throne's draw activation), then PB-R (ExchangeZones), PB-T+V, PB-U, PB-Y (Metallic Mimic parked), PB-W. Painter's Servant Tier-1-verify parked. Reports: `memory/primitives/pb-plan-Q.md`. W5 RETIRED.
...
- **Last Updated**: 2026-04-11 (PB-Q implement done; 2627 tests; review deferred to next session)
