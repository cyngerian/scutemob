...
- **Status**: 2584 tests passing; ~195 validated; 42/42 P1; 17/17 P2; 40/40 P3; 95/105 P4 (95/95 implemented; 0 planned; 9 permanent-n/a; 1 deferred: Banding post-alpha); Batch 0-16 + Mutate + Transform + Morph + Dungeon + Ring complete; Type consolidation COMPLETE; **ALL PRIMITIVE BATCHES COMPLETE (PB-0 through PB-21)**; **PB-22 deferred cleanup ALL 7 SESSIONS DONE**; W6-review 21/21 COMPLETE; 0 HIGH/MEDIUM open; **~29 LOW open**; W3 LOW sprint COMPLETE; PB-23–37 DONE; **PB-A DONE** (play from top of library); **PB-B DONE** (play from graveyard); **PB-E DONE** (mana doubling); **PB-J DONE** (copy/redirect spells: CopySpellOnStack, ChangeTargets, TargetSpellOrAbilityWithSingleTarget, 3 card fixes, 9 tests)
- **Active Plan**: **W6 Primitive + Card Authoring** — **Phase 2.5 DSL gap closure COMPLETE (PB-0–37)**. **Backfill COMPLETE**. **Wave A COMPLETE**. **Wave A+B engine reviews CLEAN**. **Wave B.5 in progress**: PB-N done (19 misc cards), **PB-G DONE** (BounceAll + 4 cards), **PB-K DONE** (land drops + Case mechanic), **PB-D DONE** (chosen creature type, 8 fixes), **PB-C DONE** (extra turns, 3 new cards, 1 fix), **PB-F DONE** (damage multiplier, 1 new card, 2 fixes), **PB-I DONE** (grant flash, 1 new card, 3 fixes), **PB-H DONE** (mass reanimate, 1 new card, 4 fixes), **PB-L DONE** (reveal/X effects, 2 new cards, 5 fixes), **PB-A DONE** (play from top of library, 6 new cards, 4 fixes), **PB-B DONE** (play from graveyard, 6 fixes), **PB-E DONE** (mana doubling, 3 new cards, 6 fixes), **PB-J DONE** (copy/redirect spells, 3 card fixes). **Next: PB-M** (Panharmonicon trigger doubling). W5 RETIRED. Goal: all 1,743 cards complete pre-alpha, zero TODOs.
...
- **Last Updated**: 2026-04-09 (PB-J: copy/redirect spells — CopySpellOnStack, ChangeTargets (CR 115.7a/d), TargetSpellOrAbilityWithSingleTarget, GameEvent::TargetsChanged, 3 card fixes, 9 tests; ~1750 total defs; 2584 tests, 0 clippy)
