...
re-filed by PB-DP10 without noticing this row. Verified `scutemob-159`, 2026-07-27:
...
### PB-OS4b — face-aware ability gathering (OOS-OS4-2) · CORRECTNESS · ✅ **SHIPPED** (`scutemob-134`, 2026-07-19)
> ✅ **SHIPPED — closes OOS-OS4-2 (correctness; jumped OS5 per the correctness-first rule).** A
> transformed permanent (`obj.is_transformed`, via in-place `TransformSelf` OR the OS4
> return/enter-transformed path) now gathers its **back face's** non-keyword abilities and no longer
> its front face's (CR 712.8d/e). Root cause was **two** front-only channels: **Channel A** (runtime
> `characteristics.{triggered,activated,mana}_abilities`, lowered from front `def.abilities` at
> construction and never rebuilt on transform) and **Channel B** (raw `def.abilities` direct-index
> sites: static registration, ETB queue, upkeep/first-main/postcombat/end-step sweeps, mana-tap
> sweep, Saga SBA, CardDefETB consumers). Fix: single `apply_face_change` choke point
> (`rules/face.rs`) does deregister-old-statics → flip → Channel-A base-rebuild
> (`build_face_ability_vectors`) → register-new-statics, routed through all 8 transform boundaries;
> Channel B routes through new `CardDefinition::effective_abilities(is_transformed)`
> (card-types) on producer + consumer, gated on live `is_transformed`.
> **Probe-by-execution outcome**: `docent_of_perfection` + `bloodline_keeper` (PB-EF5 shipped
> `Complete`) were **live-wrong pre-fix** (front cast-trigger/anthem leaked, back abilities dead) and
> are now **verified correct + STAY `Complete`**, pinned by execution tests. `growing_rites_of_itlimoc`
> + `thaumatic_compass` back mana/activated abilities now function (stay `partial`, unrelated DSL
> gaps). `fable_of_the_mirror_breaker` Reflection-of-Kiki-Jiki activated ability now
> **reachable/activatable** (message corrected; stays `partial` for ch. I/II + the Kiki-Jiki
> nonlegendary-filter residual). **Wire: NONE** (behavior-only, branches existing state — PROTOCOL 19
> / HASH 56 unchanged). Suite 3515 passing. **Deferred → OOS-OS4-3** (edgar_charmed_groom + re-add
> `Effect::ReturnSourceToBattlefieldTransformed`, a real 1-bump; not worth bundling into a wire-neutral
> correctness PB). **Documented latent (roster-unreachable) gap**: enter-transformed *replacement*
> gathering (`apply_self_etb_from_definition` / `register_permanent_replacement_abilities`) is still
...
- **✅ SHIPPED** (2026-07-19, `scutemob-137`): `silumgar_the_drifting_death` NEW→Complete via
  `EffectFilter::CreaturesControlledByDefendingPlayer` (DSL placeholder, substituted at
  `Effect::ApplyContinuousEffect` execution into `CreaturesControlledBy(ctx.defending_player)`,
...
**New seed — OOS-OS7-1 (capability)** — defending-player-scoped *target filter* +
opponent-vs-opponent attack trigger. Unblocks Karazikar, the Eye Tyrant (both halves: "tap target
creature that player controls and goad it" needs the defending-player-scoped target-VALIDATION
sibling of this PB's continuous filter; "whenever an opponent attacks another one of your
opponents" needs a new `TriggerCondition`) and the R1 half of `kogla_the_titan_ape.rs` ("destroy
target artifact/enchantment defending player controls," currently approximated as opponent).
Neither ability is expressible with PB-OS7's layer-system filter alone — R1 lives in the
target-validation path (`casting.rs`/targeting), a different subsystem. Full R1/R2 breakdown:
`memory/primitives/pb-plan-OS7.md` "Karazikar — SHIP/BLOCK decision."

**New seed — OOS-OS7-2 (correctness) — resolution-time affected-set snapshot semantics for
one-shot continuous P/T effects (CR 611.2c).** Filed by the PB-OS7 review (Finding 1, MEDIUM).
CR 611.2c: "If a continuous effect generated by the resolution of a spell or ability modifies the
characteristics ... of any objects, the set of objects it affects is determined when that
continuous effect begins. After that point, the set won't change." The engine's
`CreaturesControlledBy(pid)` (and every filter shaped like it — `AllCreatures`,
`AllCreaturesExcludingSubtype`, etc.) locks only the *player*/predicate at resolution; `filter_matches`
re-evaluates *membership* live at every `calculate_characteristics` call, so a creature entering or
leaving the matched player's control mid-turn is wrongly swept in or dropped out of the effect.
This is **engine-wide and pre-existing**, not introduced by PB-OS7 — confirmed present in
`golgari_charm.rs` (`AllCreatures`), `eyeblight_massacre.rs` (`AllCreaturesExcludingSubtype`), and
now `silumgar_the_drifting_death.rs` (`CreaturesControlledByDefendingPlayer` →
`CreaturesControlledBy`). Fix shape (sketch, NOT scoped): a resolution-time snapshot mechanism that
captures the matching `ObjectId` set once when the continuous effect begins (CR 611.2c) rather than
storing a re-evaluable predicate — e.g. a new `EffectFilter::ObjectIdSet(im::OrdSet<ObjectId>)`
variant that `ApplyContinuousEffect` populates by evaluating the predicate ONCE against the current
battlefield at execution time, with `filter_matches` doing simple set membership instead of a live
predicate re-check. Touches every resolution-generated (non-static) `ApplyContinuousEffect` filter
site engine-wide — a genuine subsystem-level primitive batch, not a card-specific fix. Candidates:
the whole resolution-generated mass-pump/mass-debuff card family (Golgari Charm, Eyeblight
Massacre, Silumgar, and any future card using a description-based `EffectFilter` off a triggered/
