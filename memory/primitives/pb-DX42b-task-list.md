...
- [x] 2.2 UNDER-count direction on a real `LocalGame`/`HumanChoice` drive — **OUT OF SCOPE for
      this task** (assigned to a second agent per the dispatch brief)
- [x] 2.3 OVER-count direction on the same channel — **OUT OF SCOPE**, second agent
- [x] 2.4 `thaumatic_compass` DFC case as its own test — **OUT OF SCOPE**, second agent
- [x] 2.5 "two distinct conditional effects nest without mutual suppression" probe —
      **OUT OF SCOPE**, second agent
- [x] 2.6 `debug_assert` fires on a synthetic same-layer case — test — **OUT OF SCOPE**,
      second agent
...
- [x] 2.8 Rider `OOS-ADJ-2` (`pb_dx42a_continuous_condition_roster`) — **OUT OF SCOPE**, second
...
- [x] 3.1 Post-run full workspace; delta by test NAME via byte-exact set difference (`OOS-DX20b-5`)
- [x] 3.2 Count-vs-name reconciliation + duplicate-name scan (`OOS-DX35-8`); leavers/renames disclosed
