...
**Membership semantics (design note for the runner)**: We store `CreaturesControlledBy(pid)` where
`pid` is locked at resolution, but the *membership* (which creatures that player controls) is
evaluated live by `filter_matches` at each `calculate_characteristics`. This matches how every
other continuous P/T filter behaves and is CR-correct for "creatures defending player controls get
-1/-1 until end of turn": if the defending player gains a creature later this turn, that creature
is affected — standard behavior for a description-based continuous effect. Only the PLAYER identity
is captured; that is exactly what the per-attacker `defending_player_id` capture supplies.
...
**Optional adjacent seed OOS-OS7-2 (capability, low priority)** — `CreaturesControlledByTargetPlayer
{ index }` continuous placeholder for polymorphists_jest / great_oak_guardian / naya_charm. Same
one-line substitution shape as this PB. Note only; do not author.
...
- **Membership vs player capture (CR 611.2c nuance)**: we lock the PLAYER at resolution but let the
  filter re-evaluate membership live. Correct for "creatures defending player controls get -1/-1
  until end of turn." Do NOT try to snapshot the creature set — that would be wrong and is not how
  any existing P/T filter works.
