...
| coverage, regenerated by `tools/authoring-report.py` | **1,136 / 1,803 = 63.0%**, clean 1,136 / todo 520 / empty 147 — **byte-identical counts to PB-DX44's close, 0 flips**, self-dating churn reverted. **0 card-def edits**, so the only diff was the timestamp, the SHA and the rolling activity windows |
