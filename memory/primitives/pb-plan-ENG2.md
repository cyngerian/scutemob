...
| `OOS-ENG2-5` | Three shipped comments cite **CR 108.1** (the Oracle-text rule) for "a player target is public". **CLOSED by the §4.6 rider.** The rider's own first replacement, `CR 102.1 / 115.1 / 400.2`, was itself weak and was corrected in the fix cycle: 102.1 merely defines what a player *is* and 400.2 is about card faces in a zone, so neither speaks to a player being a legal, publicly-announced target. The shipped chain is **CR 601.2c** ("an appropriate object **or player** for each target" — the on-point rule, and the one this batch's own title cites) **/ CR 400.2** (the stack is a public zone, which is *why* the announcement is visible). Six sites, not three. | — |
