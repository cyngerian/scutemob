...
| **7b** | **PB-DX25b** *(INSERTED 2026-08-06, user-approved — filed after this memo's census by PB-DX25 itself, so it holds a slot by the same rule that ranked its siblings, not by silent repoint)* | `validate_target_requirement`'s spell-target id-space confusion — the `OOS-SIM3-5` defect one function over | **OOS-DX25-3** | **CORRECTNESS — LIVE on 2 deck-legal `Complete` defs** (`misdirection`, `bolt_bend` can never resolve a legal target: `casting.rs:6426` resolves the announced id via `state.objects` (card space) then `:6476`/`:6502` compare it to `so.id` (stack-entry space) — one monotone counter mints both, so the match is impossible; the negative tests at `casting.rs:8188-8321` pass vacuously because the requirement refuses everything) | 0 flips; repairs 2 `Complete` defs + de-vacuates the negative tests; the natural fix consumes PB-DX25's `state::stack_registry` classification, freshest now | **none** predicted (validation-site only; no stored state, no `Command`/`GameEvent`/`Effect` change) — gate-execute anyway |
