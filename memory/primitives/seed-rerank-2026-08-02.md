...
| ~~**8**~~ | ~~**PB-DX26**~~ **✅ SHIPPED** (`scutemob-206`, 2026-08-11) | the equip surface, one link earlier | **OOS-CARDS1-3**, **OOS-CARDS1-1**, **OOS-DX3b-1** — **all three CLOSED** | **CORRECTNESS + CARD YIELD** | **1 flip, not ~4-6** — the estimate was wrong in a way worth recording: the 10 deck-legal defs were ALREADY `Complete` (nine by the `#[default]` derive), so repairing them is a repair-in-place with 0 flips, and the single flip came from the *other* half of the roster — `sword_of_body_and_mind`, whose `partial` note named the missing Equip {2} as its only blocker. Coverage **1,133/1,803 = 62.8% → 1,134/1,803 = 62.9%**. §2.7's method warning was correct and was followed: re-derived from `all_cards()`, **21** total / **10** deck-legal `Complete`, both exact. | **none** — 0 engine-source lines; no `keyword_registry` promotion was needed |
