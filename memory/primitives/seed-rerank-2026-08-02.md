...
| `OOS-SIM3-5(c)` | 6 mutate defs × 24 counter defs | 6 `Complete` mutate defs — **CORRECTED by PB-DX25 (`scutemob-203`), measured from `all_cards()` rather than grep: 6 × 8 = 48 pairs, not 144.** Only a counter whose requirement admits a *creature* spell can reach a mutate spell, and exactly **8** `Complete` defs carry unrestricted `TargetRequirement::TargetSpell`. Also: the counter-def population is **23**, not 24 — the 24th was a grep matching the literal text `Effect::CounterSpell` inside a **TODO comment** on `transcendent_dragon.rs`, which carries no such effect in code |
