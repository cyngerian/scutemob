...
| **OOS-SIM3-5** | states (b) "countering a copy moves the original's card" as a flat fact | **(b) is unreachable at HEAD**: the lookup is `position()` (first match = lowest index = bottom of stack), so with the original still on the stack a counter always lands on the original; with it gone, `TargetSpell` validation refuses. Meanwhile the row's **(c) rider** — a plain Counterspell on a mutate spell is a silent no-op — is the one that is live, on 6 `Complete` mutate defs × 24 counter defs | **rank on (c), fix all three in one edit** — done, PB-DX25 (`scutemob-203`). **Two corrections to this correction, both measured by that batch.** (1) The unreachability of (b) is **three** mechanisms, not one: the ordering argument above, *plus* `resolve_effect_target_list_indexed` dropping a dead `DeclaredTarget` (so the window is empty, not narrow, even under CR 608.2b re-validation), *plus* nothing aiming a counter at a copy at all — a copy's stack-entry id is not a `state.objects` key, so `TargetSpell` refuses it **always**, not merely once the original is gone. (2) **(a) is not independently live either**, and this row's "fix all three in one edit" was righter than it knew: Ward cannot reach a mutate spell, because the mutate target is carried in `AdditionalCost::Mutate` and never enters `spell_targets` (`OOS-DX25-1`), so **(a) is what fixing (c) ALONE would have created** — a permanent `ZoneId::Stack` leak in place of a silent no-op |
...
| `OOS-SIM3-5(c)` | 6 mutate defs × 24 counter defs | 6 `Complete` mutate defs — **CORRECTED by PB-DX25 (`scutemob-203`), measured from `all_cards()` rather than grep: 6 × 11 = 66 pairs, not 144.** Only a counter whose requirement admits a *creature* spell can reach a mutate spell; **11** `Complete` defs qualify. The counter-def population is **29**, not 24, and getting there took two corrections in opposite directions, both worth recording. **Down:** the grep's 24th match was the literal text `Effect::CounterSpell` inside a **TODO comment** on `transcendent_dragon.rs`, which carries no such effect in code. **Up, and this was the batch's own review catching the batch:** the first enumeration walked `Effect::CounterSpell` alone and was structurally blind to **`Effect::CounterUnlessPays`**, which `effects/mod.rs` delegates *straight into the same arm* — so `mana_leak`, `mana_tithe` and `make_disappear` (all `Complete`, all printing plain "counter target spell unless…", all carrying `TargetSpellWithFilter(TargetFilter::default())`, which is unrestricted field-by-field) were equally live-wrong and equally invisible. It also missed counters on activated/triggered abilities and back faces. **An SR-36 enumeration is only as wide as the variant list it walks** — the first re-measure published **48** and was wrong for a subtler reason than the grep it replaced |
...
|---|---|---|---|
| ~~**PB-DX7**~~ **✅ SHIPPED** (`scutemob-207`, 2026-08-11) | `OOS-DP7-11` + `OOS-DP9-13` (+DP10-1, DP9-10 residual) — **all four CLOSED** | ~~**SURVIVES, rank 9**~~ **DONE** | Still real, still test-only, still 0 flips — all three held. **The "still test-only" prediction held only because the batch chose to hold it**: three of the five closures were found by widening past the brief, and one of them (`OOS-DX7-1`, the `Effect` discriminant reuse) could not be *repaired* without moving the byte stream, so it is filed rather than fixed. See the §4 rank-9 row for the full account, including the enum scope being 8× the brief's and the two revert rows that caught bugs in the new gate before it shipped. |
...
| **6** | ~~**PB-DX24**~~ ✅ SHIPPED (`scutemob-202`, 2026-08-05) | the lowering drops `trigger_zone`; the two index spaces disagree | **OOS-DX1-3** + **OOS-DX1-4** — **both CLOSED** | **CORRECTNESS — live-wrong on `nether_traitor`** | 0 flips as predicted (**1,133/1,803 = 62.8%** unmoved, proven by regenerating the report to an identical body, not by an empty diff — the batch mandates a comment-only def edit); 1 live def repaired, 6 queue sites aligned, Q5 re-scoped on CR 712.2 | **none** — narrow fix taken, PROTOCOL **35** / HASH **73** gate-executed and unmoved. **The row's scope was short by one half**: the brief and this row both read the fix as a one-line skip, but `collect_graveyard_carddef_triggers` dispatched `PermanentEnteredBattlefield` ONLY, so the skip alone would have shipped a card that fires **nowhere**. **And the "6 latent queue sites" number was right for the wrong reason** — SR-36 measured **0** corpus defs with any Q1-Q7 shape on a back face, so all 7 are latent and every probe is synthetic. Filed **OOS-DX24-1..6**; **OOS-DX24-1** is a NEW live-wrong instance this batch introduced (a CR 603.2d death doubler now doubles a graveyard-sourced trigger — `teysa_karlov`, `drivnod_carnage_dominus`), deferred deliberately, not overlooked |
| **7** | ~~**PB-DX25**~~ ✅ SHIPPED (`scutemob-203`, 2026-08-05) | `Effect::CounterSpell`'s three stack-object shapes | **OOS-SIM3-5** — **CLOSED** | **CORRECTNESS — a countered spell resolves anyway, silently** | 0 flips as predicted (**1,133/1,803 = 62.8%**, proven by *regeneration*, not by the empty card-defs diff the plan would have accepted); **66** live-wrong pairs, not the 144 this row implied — see the §3 row for the two corrections, in opposite directions, that got there | **none**, gate-executed (PROTOCOL **35** / HASH **73** unmoved) — the prediction held. **This row's framing was wrong about which shape was live, and the seed's was too.** (c) was the only live shape; **(a) was not independently reachable** — Ward cannot reach a mutate spell, because the mutate target never enters `spell_targets` (`OOS-DX25-1`) — so **(a) is what fixing (c) ALONE would have created**, a permanent `ZoneId::Stack` leak in place of a silent no-op. "Fix all three in one edit" was righter than it knew. (b) is unreachable **three** ways. **The fix is structural, not three patches**: one engine-side `state::stack_registry::card_in_stack_zone`, exhaustive with no wildcard, consumed by **both** counter paths, so a 28th `StackObjectKind` is a compile error until classified. The simulator's `stack_card_of` is **deliberately NOT unified** with it — a verifier that reads the engine's own answer goes silent on exactly the defect it exists to catch. Review 0 HIGH / 6 MEDIUM / 3 LOW + 7 folded notes, **all taken**; its three sharpest findings were this batch's own failure mode recurring inside it (a census short by two sites, one of them wrong in the *same direction* as the defect; a roster blind to a delegating variant; a "non-vacuity" assertion comparing a fixture to itself). Filed **OOS-DX25-1..6** |
| **7b** | ~~**PB-DX25b**~~ ✅ SHIPPED (`scutemob-204`, 2026-08-06) *(INSERTED 2026-08-06, user-approved — filed after this memo's census by PB-DX25 itself, so it holds a slot by the same rule that ranked its siblings, not by silent repoint)* | `validate_object_satisfies_requirement`'s spell-target id-space confusion — the `OOS-SIM3-5` defect one function over | **OOS-DX25-3** — **CLOSED** | **CORRECTNESS — LIVE on 2 deck-legal `Complete` defs** (`misdirection`, `bolt_bend` can never resolve a legal target: the announced id is resolved via `state.objects` (card space) then compared to `so.id` (stack-entry space) — one monotone counter mints both, so the match is impossible; the in-src tests at `casting.rs:8188-8321` pass vacuously because their fixture **collapses the two id spaces**, a configuration no real cast can produce) | 0 flips as predicted (**1,133/1,803 = 62.8%**, proven by *regeneration*; all 4 card-def edits comment-only, verified per-line). Tests **4,452 → 4,469 (+17)**, residual empty | **none**, gate-executed (PROTOCOL **35** / HASH **73** unmoved) — the prediction held. **But this row's "validation-site only" was wrong, and a fix obeying it would have shipped a cast that announces and then silently does nothing.** `Effect::ChangeTargets` — the effect *both* cards use — matches the same announced id against stack-entry ids at **three** more places; `Effect::CopySpellOnStack` is a fourth (latent) and `Effect::CounterSpell` had open-coded the correct rule as a fifth. **The fix is structural**: one `state::stack_registry::stack_index_for_announced_target` (`so.id == announced \|\| (!so.is_copy && card_in_stack_zone(..) == Some(announced))`) encoding the rule ONCE, consumed by all five; the `!so.is_copy` guard is load-bearing twice (disambiguation + the CR 702.99c cipher-copy exile leak). The `is_spell` guard is KEPT though now production-unreachable — deleting it would be CR-wrong the day `OOS-DX25b-1` closes. Review 1 HIGH / 5 MEDIUM / 6 LOW, **all 12 taken**; the HIGH was **a plan deliverable the implement phase silently dropped** (the CR 115.7a wrong-way-round probe) plus an execution note that misstated the plan as having deferred it, and the reviewer **defeated the R5 gate three ways** — two now caught, the third stated as a permanent structural residual rather than overclaimed. The reviewer also re-derived the census by the inverse method and confirmed **no sixth site**. Filed **OOS-DX25b-1..5**, of which **OOS-DX25b-3 is LIVE on the same 2 `Complete` defs** — this batch is what makes CR 115.7a's unchecked object-target redirect reachable |
| **7c** | ~~**PB-DX25c**~~ ✅ SHIPPED (`scutemob-205`, 2026-08-06) | the object-target redirect ignores CR 115.7a's "another LEGAL target" | **OOS-DX25b-3** — **CLOSED** | **CORRECTNESS — was live game state from legal play on 2 deck-legal `Complete` defs, reachable by humans AND bots** — new `StackObject.target_requirements` (hashed) + `rules::retarget::plan_target_change`, delegating the whole redirect decision to `casting::validate_targets_inner` (CR 115.3/115.7e/115.7a all-or-nothing); the PLAYER branch's matching defect (no requirement check, `has_lost`-only) was ALSO closed in the same fix, not named by this row originally | 0 flips (**1,133/1,803 = 62.8%**, proven by regeneration; both card-def edits comment-only, verified per-line); inverted `t9` (renamed `...obeys_the_original_requirement`) + new `t9b`; +22 tests across the implement + two fix cycles (4,469 → 4,491, re-measured at fix cycle 3), residual empty | **HASH 73 → 74**, gate-computed; PROTOCOL **35** unmoved, gate-executed. Filed **OOS-DX25c-1..6** (OOS-DX25c-5 CLOSED, fix cycle 2) |
| ~~**8**~~ | ~~**PB-DX26**~~ **✅ SHIPPED** (`scutemob-206`, 2026-08-11) | the equip surface, one link earlier | **OOS-CARDS1-3**, **OOS-CARDS1-1**, **OOS-DX3b-1** — **all three CLOSED** | **CORRECTNESS + CARD YIELD** | **NET ZERO flips, not ~4-6** — the estimate was wrong twice over, in opposite directions. The 10 deck-legal defs were ALREADY `Complete` (nine by the `#[default]` derive), so repairing them is a repair-in-place with 0 flips; one flip UP came from the *other* half of the roster (`sword_of_body_and_mind`, whose `partial` note named the missing Equip {2} as its only blocker); and one flip DOWN came from the batch's own `/review` (`the_reaver_cleaver` — derive-`Complete` with no marker anyone had ruled on, granting a trigger that under-fires against "a player or planeswalker"). Coverage **unmoved at 1,133/1,803 = 62.8%** — and the pair is the batch's sharpest lesson for a future estimator: **a card-yield estimate that counts defs to REPAIR is measuring the wrong thing**, because a repair on an already-`Complete` def moves nothing while a review of that same def's unexamined marker can move it down. §2.7's method warning was correct and was followed: re-derived from `all_cards()`, **21** total / **10** deck-legal `Complete`, both exact. | **none** — 0 engine-source lines; no `keyword_registry` promotion was needed |
| ~~**9**~~ | ~~**PB-DX7**~~ **✅ SHIPPED** (`scutemob-207`, 2026-08-11) | SR-19 gate holes | **OOS-DP7-11** + **OOS-DP9-13** — **both CLOSED**; riders **OOS-DP10-1 CLOSED** and **OOS-DP9-10 residual CLOSED** (gated, not deferred) | gate integrity | **0 flips as predicted** (1,133/1,803 = 62.8% unmoved, proven by *regeneration* with the self-dating churn reverted, not by the empty card-defs diff alone); test-only held exactly — **0 non-comment lines** in `hash.rs`, verified line-by-line in python because `grep -E '^[+-]//'` gives a **false positive on indented comments**. Tests 4,508 → **4,524 (+16)**, delta itemised by test NAME rather than arithmetic. | **none**, gate-EXECUTED (PROTOCOL **35** / HASH **74** unmoved) — the prediction held. **But "5 structs + all hashed enums" undersold the enum half by a factor of 8, and this row's own bundling was the reason.** The brief scoped the enum work to the *10 path-qualified* enums; path qualification is irrelevant there — **all 79** hashed enums were outside the struct gate, so obeying the row would have closed `OOS-DP9-13` on paper while 69 enums stayed uncovered. Final scope: 79 enums / **1,252 variants** / **1,097 variant fields**. **Three further holes of the same family surfaced only by widening, all closed here rather than filed forward**: the coverage predicate could not tell `self.f.hash_into()` from `self.f.is_some().hash_into()`, so 4 sites passed as covered while their values never reached the hasher (`OOS-DX7-2`); `Effect` reuses **9 discriminants across 18 variants** while its comments called them unique, settled by an **executed** pairwise-distinctness experiment after the implementer's "subsequent bytes differ" assertion was rejected, plus a ratchet so a 10th cannot appear (`OOS-DX7-1`); and **`GameState` was carved out of the field gate entirely**, with 3 of its 45 fields reaching no hash and no stated exclusion list (`OOS-DX7-3`). **24 revert rows, all executed red then restored, none UNDISCRIMINATED** — and two found real bugs *before* shipping: V14 exposed a false negative in the new dead-entry checker (it searched for the literal index string `"0"` instead of the actual pattern binding, passing GREEN where it should have failed RED — this batch's own subject matter recurring inside its implementation), and V18 forced an artificial digest collision rather than assuming the detector fired. Corrections carried back: the seed's cite `hash_schema.rs:1540-1541` names the wrong symbol; the implement phase's "26 revert rows" is **24**; and a premise the coordinator asserted — that `HASH_SCHEMA_HISTORY` rows inherited the discriminant error — was **checked and found false**, so a clean-result note was recorded instead of a correction being invented to fit the instruction |
| ~~**10**~~ | ~~**PB-DX8**~~ **✅ SHIPPED** (`scutemob-208`, 2026-08-12) | oracle-text-vs-DSL cross-check | **OOS-DP10-9** — **RECORDED, not closed** (the gate makes the class visible and says so in its own failure message; the class itself needs the owning engine PB, audit §5 DP-12) · **OOS-CARDS2-7** — **FILED and CLOSED** (it had no §8.1 registry row at all until this batch wrote one; grep-confirmed absent first, per dispatch hygiene 5) · rider **PB-DX42a** SHIPPED per adjudication §5.1 | gate integrity — the worst blind spot, now measured | **0 flips as predicted** (1,133/1,803 = 62.8% unmoved, proven by *regeneration* with the self-dating churn reverted). **Test-only held exactly: 0 lines across `crates/engine/src`, `crates/card-types/src`, `crates/card-defs/src`, `crates/view-model/src`, `crates/simulator/src` and `tools/`** — and 0 card-def edits of any kind, comment or otherwise. **The row's "35 `Complete` gap-noted defs" reproduces EXACTLY at HEAD** by the seed's own six needles, so the filed figure was neither stale nor an estimate — but the derived set is **not a superset of it**: the two are non-nested (14 seed-only, 10 derived-only), and the shipped population is their measured union, **45**. | **none**, gate-EXECUTED (PROTOCOL **35** / HASH **74** unmoved; `hash_schema` 36/36, `protocol_schema` 17/17). **The batch's subject matter recurred inside its own implementation three times, and every instance was caught by execution rather than by reading.** (1) The oracle axis read `def.oracle_text` alone while `CardFace` carries its **own** `oracle_text` — blind to every transformed face and every Adventure half, found by the inverse-method census, not by any test (`OOS-DX8-4`). (2) The deviation scan matched the whole SOURCE file while its needle set was derived from PROSE, so `drawcards` (== `Effect::DrawCards`) scored **203 files / 127 unmarked / 37% precision** against the derivation's own **20 / 1 / 95%** — it could never have survived D2 under a fair measurement (`OOS-DX8-6`). (3) The first-draft population ratchet filtered its own denominator down to the roster it was checking and **could never redden**, caught only because revert row V4 had to demonstrate red (`OOS-DX8-7`). **And the derivation itself failed three ways before it worked**, all measured: iterated bootstrapping drifts into search vocabulary (`battlefield`, `library`, `graveyard` by iteration 3); single-pass lift returns object nouns; and a vocabulary learned from the DSL's own ground truth is **self-blinding on the target** — it drops `may` entirely, because "you may" is precisely the class the DSL cannot encode (`OOS-DX8-5`). Populations: `may` **285** oracle-positive / **72** effectively-`Complete` with no may-shaped construct, `choose` 116 / 2, `up_to` 70 / 10; union **80**, frozen and ratcheted. Filed **OOS-DX8-1..8**. Revert matrix **30 rows across the three files** (12 + 8 + 10), all executed; **29 RED, 1 honestly UNDISCRIMINATED** (`PROSE_FIELDS`, inherited rather than earned — disclosed in the module doc rather than implied). **The `/review` cycle then found 4 MEDIUM / 6 LOW, all 10 taken, and DEFEATED two of the three gates by execution**: the deviation scan's narrowing to `//` comments silently dropped `/* */` blocks (the byte-identical sentence red as a line comment, green as a block comment — `OOS-DX32-6`'s class, latent only because the corpus carries zero such comments today), and the cross-check's evidence is scoped to the **def** rather than the **clause**, so a printed "may" on a def whose `optional: true` belongs to an unrelated clause is invisible — **24** `Complete` defs are exempted by a single piece of evidence, now stated as a second recall bound. Also taken: a doc citing a test **that did not exist**; a **compile-time-tautological** test (`A || true`) whose rewrite now reddens 5 tests on a one-field desync where it reddened none; and two published numbers that were the **pre-fix** figures republished in the documents celebrating the fix that invalidated them (`may` 285 → **287**, suppression 18 → **24**) — corrected, and a `t_reconciliation_report` added so the code PRINTS them, which is the durable half. Tests **4,561** (+34). |
| ~~**11**~~ | ~~**PB-DX27**~~ **✅ SHIPPED** (`scutemob-209`, 2026-08-13) | the stale-blocker-note sweep + the wrong-oracle register | **OOS-CARDS2-8** + **OOS-CARDS2-11** + **OOS-CARDS2-10** + **OOS-RR3-2** — **all four CLOSED**, plus the rider **OOS-ADJ-7 CLOSED**; **none of the five had a registry row** until this batch wrote one (grep-confirmed absent — the third batch running to find its own seeds unrowed) | **CARD YIELD + CORRECTNESS** | **+3 net flips** — inside the ~4-8 estimate but not by the route it predicted: **UP** `chord_of_calling`, `reconnaissance`, `wight_of_the_reliquary`, `chandra_flamecaller`; **DOWN** `qarsi_sadist` (no `TriggerCondition::WhenThisExploitsACreature` — it was the only one of the corpus's three Exploit defs claiming `Complete`) and `green_suns_zenith` (promoted by the implement phase, **demoted by the review**). Coverage 62.8% → **63.0%** (1,133 → 1,136), regenerated. Tests 4,561 → **4,605 (+44)**, itemised by name, 0 removals | **BOTH — and this row's "none (card-def)" was WRONG.** PROTOCOL **35 → 36**, HASH **74 → 75**, both gate-computed. The rider needed a new `LayerModification::SetLandTypes`, and `ContinuousEffectDef.modification` is a sibling of `filter`/`duration`, both already in the `Command`/`GameEvent` closure — so `LayerModification` is on the wire. The dispatch brief predicted "expected wire impact NONE" and `protocol_schema_fingerprint_is_pinned` refuted it. **The row's "67 machine-checkable notes" also does not reproduce**: 49 by its own literal method at HEAD, 46 ground-truth-restricted, 109 by an inverse method, and no variant reaches 67 — a snapshot, and **NOT the floor** the brief called it, since every reproduction is *smaller*. **Its "2 live-wrong `Complete` defs" undersold the rider**: `OOS-ADJ-7`'s population is **3** (`dryad_arbor` was missed) and the same ruling's `{T}: Add {R}` clause was implemented **nowhere**. **`OOS-ADJ-2` came true on PB-DX42a's first real event** — authoring The World Tree's six-lands static grew the layer-querying population **1 → 2**, and the gate fired rather than absorbing it, so **PB-DX42b's rank premise (a measured population of exactly 1) is now false** (`OOS-DX27-9`). Two adjudicated-REFUTED repairs were **declined** because existence is not sufficiency (`OOS-DX27-3`, `-4`). Review **1 HIGH / 5 MEDIUM / 6 LOW, all 12 taken**; the HIGH was the batch committing its own subject matter **twice on the same two defs** — "then shuffle" left unauthored against the in-source warning of the precedent they cite. Filed **OOS-DX27-1..10** |
| ~~**12**~~ | ~~**PB-DX28**~~ **✅ SHIPPED** (`scutemob-210`, 2026-08-14) | the untargeted-choice class + the owner axis | **OOS-DX4-6** + **OOS-DX4-1** — **both CLOSED** (registry rows updated; filed **OOS-DX28-1..8**) | **CORRECTNESS — exploitable, ≥14 `Complete` defs incl. 10 Karoos** | **0 flips as estimated** (coverage 1,136/1,803 = 63.0% by regeneration). Part 1: `TargetFilter` owner axis + `EffectTarget::DamagedPlayer`. Part 2: the choose-on-resolution channel — `ChoiceZone`, `EffectTarget::ChosenObject`, `EffectChoiceQuestion/Answer::ChooseObject` — with a dedicated `filter_matches_object_untargeted` predicate, deliberately NOT `casting::validate_targets_inner`. Tests 4,605 → **4,634** (+29 by name, 0 removals). Batch ran in two parts around a system reboot with a disclosed non-compiling WIP checkpoint (`92ea0ec2`), resumed from `pb-DX28-RESUME.md` with no number re-guessed. /review 3 MEDIUM / 4 LOW, all 7 taken — the reviewer defeated two of the batch's own gates by execution and caught two fingerprints in the evidence record that never existed in the repository. Two findings past the plan, both seed-filed: the auto-target picker is TWO functions, not one; and `pb_dx42a`'s `TARGET_FILTER_FIELDS` fingerprint went blind corpus-wide on a routine field addition (one axis 2 members → 0, no compile error). Full record: `memory/primitives/pb-DX28-execution-notes.md` | **both moved as this row predicted, one bump each**: PROTOCOL **36 → 37** / HASH **75 → 76**, gate-computed |
| ~~**13**~~ | ~~**PB-DX29**~~ **✅ SHIPPED** (`scutemob-211`, 2026-08-14) | the `params.rs` allowlist and the cost-kind surface | **OOS-M11-10(loyalty)** + **OOS-UI2-4** — **both CLOSED**; the OOS-M11-10 **ID collision is RESOLVED** (equip seed renumbered `OOS-M11-10E`), per the note's own instruction to whichever task next touched `params.rs`. Filed **OOS-DX29-1..17** | **AGENCY — and the population was 4 of SEVEN `Complete` planeswalkers, not 4 of 6.** A fifth, `chandra_flamecaller`, was live-wrong on the X half with no cite naming it: `LoyaltyCost::MinusX` + `EffectAmount::XValue` resolved as **−0 for 0 damage** in every client. On the cost side, **"13 of 15 kinds" is arithmetically right and materially misleading** — four have no deck-legal member at all and three more are unreachable by construction; 9 of 15 are now surfaced and the other 6 are deferred with a stated reason each | **0 flips as predicted**, and the three card-def repairs are all on defs whose marker was already right or already `partial` | **THIS ROW'S "none" WAS WRONG, and so was the seed's.** The seed says CR 602.2b targets are "already reachable through `queries.rs::ability_target_requirements`' sibling path" — true of the machinery, **false of the index space**: that function indexes `Characteristics::activated_abilities` while a loyalty `ability_index` is minted against the REGISTRY def's filtered `LoyaltyAbility` list. Two new read-only engine queries were needed. **No type moved and no wire moved** (PROTOCOL/HASH gate-executed), so the row was right about the *consequence* and wrong about the *cost*. **The enforcement-site list was short by three**, in two crates — including `targeting.rs`, the BOT path, whose omission would have re-created SIM-5's zero-target defect on a new action. And the cost half's "a provider + view + picker addition" omitted the load-bearing link: `effective_cast_cost_with_additional` read Squad and nothing else, and it is what the auto-tap asks how much to tap, so shipping the pickers alone would have created the exact SR-38 defect the batch was dispatched to remove |
