...
| **7c** | **PB-DX25c** *(INSERTED 2026-08-06, user-approved — the successor batch OOS-DX25b-3's own wrong-way-round pin `t9_object_target_redirect_ignores_the_original_requirement` instructs; filed after this memo's census, slot held by the same rule as 7b)* | the object-target redirect ignores CR 115.7a's "another LEGAL target" | **OOS-DX25b-3** | **CORRECTNESS — wrong game state from legal play, LIVE on 2 deck-legal `Complete` defs, reachable by humans AND bots** (`Effect::ChangeTargets`'s `Target::Object` branch, `effects/mod.rs:7619-7654`, picks the lowest-`ObjectId` object in `zone_at_cast` with no requirement/protection/hexproof/shroud/type check — a Misdirected "destroy target creature" destroys a basic land; made reachable by PB-DX25b, and it pollutes fuzz evidence since the fuzzer casts real spells post-DX22) | 0 flips; inverts the t9 pin; closes the redirect-legality gap for both `Complete` defs | **HASH 73 → 74 predicted** (the victim spell's `TargetRequirement` list rides on `StackObject`, a hashed field — the registry row scopes this as a batch of its own) — gate-compute, never predict-only; PROTOCOL expected unmoved |
