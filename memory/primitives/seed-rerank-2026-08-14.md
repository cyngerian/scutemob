...
> The banner below it is superseded on the 'NO FURTHER DISPATCH' point only.**
>
> **↻ 2026-09-05: `PB-DX55` SHIPPED (`scutemob-234`) — task 1 of 5 of that chain, and rank 19 is
> struck below. All three of its seeds — `OOS-SIM6-3`, `OOS-SIM5-3`, `OOS-SIM5-5` — were FILED
> into the registry by that batch (none had a row) and then CLOSED, together with the rider
> `OOS-DX51-3`. The bot refusal surface goes **70 → 9** and every survivor is one class.
> ~~The next dispatch is `PB-DX56` (rank 20).~~ *(superseded — `PB-DX56` shipped `scutemob-235` 2026-09-05.)* ~~**The next dispatch is `PB-DX57` (rank 21), task 3 of 5 of the second chain.**~~
>
> **↻ 2026-09-05: `PB-DX57` SHIPPED (`scutemob-236`) — task 3 of 5 of the second chain, and rank
> 21 is struck below. `OOS-DX28-1` and `OOS-DX28-6` CLOSED **as classes** (35 fingerprint members
...
> without explicit user approval: `feedback_queue_autonomous_chaining` was RETRACTED on
> 2026-08-01 and every dispatch, including restarting a crashed worker, needs it.
>
> **↻ 2026-09-05 (historical): `PB-DX54` SHIPPED (`scutemob-232`); the next dispatch was
> `PB-DX42b`** (rank 18 — CR 613.1d layer-bounded condition queries, `OOS-ADJ-1` ≡ `OOS-DX19-2`
> plus `OOS-DX19-1`'s residue).
>
...
>
> **Read this before dispatching from row 16's cell.** Two of its three cells were REFUTED and the
> refutations are the reusable part: a wire cell that names only the field you expect to add says
> nothing about the IDENTIFIER you may have to split, and a "1 deck-legal `Complete`" yield cell is
> a floor whenever the class is defined by a printed phrase rather than by a declared variant —
> `minas_tirith` was invisible to the declared axis because its ability was UNAUTHORED, and only
> the inverse ORACLE axis finds a card whose defect is that it is missing.
>
> **Read this before dispatching from row 15's cell.** The wire cell's *"none if derivable at
> resolution"* branch fired, but its implied preference for the OTHER branch would have shipped a
> CR-wrong fix — a snapshot stored at activation cannot answer the Jitte ruling's *"most recently
> equipped"*. The yield cell's **0 flips** held exactly. What the memo could not tell you: (a)
> *"narrow, one card"* is short by 15× — the resolution-time class is 21 deck-legal defs and the
> largest axis is a RACE class no document names; (b) the seed's own preferred fix does not work
> as written, because SR-24 leaves `lki_objects` EMPTY for both subjects, so the batch is *read
> the LKI **and** make it carry the source*, with TWO capture clauses because the two subjects
> leave at different moments; (c) **a class fix repairs the arithmetic, not every caller's route
> to it** — `binding_the_old_gods` is deck-legal, is in the class, and is still broken one link
> upstream (`OOS-DX39-3`), which also refutes PB-DX49's own recorded diagnosis of that card.
>
> **Read this before dispatching from row 14's cell — every one of its cells held, and the two
> things that did NOT come from the memo are the ones worth carrying.** The wire cell was right
> on both halves and at HIGH confidence for once (`Target` and `TargetRequirement` are both in
> the closure; both counts predicted and confirmed unchanged at 98 / 132). The yield cell's
> **0 flips** held. What the memo could not tell you: (a) the "1 deck-legal `Complete`" figure
> is a FLOOR — **four** defs declare a stack-object requirement, and closing the seed is what
> makes `misdirection`'s spell-only restriction enforceable for the first time, because
> `TargetSpellWithSingleTarget` and `TargetSpellOrAbilityWithSingleTarget` were behaviourally
> IDENTICAL on every production path; and (b) **a seed's "unreachable today" is a claim with an
> expiry date, and the batch that closes its blocker is the batch that has to honour it** —
> `OOS-DX25c-3` was filed as doubly-unreachable *because* `OOS-DX25b-1` was open, so closing
> `OOS-DX25b-1` without also closing it would have shipped a CR 702.16b protection bypass. When
> dispatching any seed, grep the registry for rows whose stated blocker is the seed you are
> about to close.
>
...
| ~~**14**~~ | ~~**PB-DX52**~~ **✅ SHIPPED** (`scutemob-229`, 2026-09-04) | Bolt Bend's printed "or ability" half is unreachable | **OOS-DX25b-1** + **OOS-DX25b-5** (rider) — **both CLOSED**, plus **OOS-DX25c-3** CLOSED as a third | correctness — **the "1 deck-legal `Complete`" cell is a FLOOR**: four defs declare a stack-object requirement (`bolt_bend`, `misdirection`, `deflecting_swat`, `untimely_malfunction`) and `misdirection`'s spell-only restriction was unenforceable for the same reason | **0 flips — EXACTLY right** (coverage unmoved 1,139/1,803 = 63.2%, proven by regeneration; no `Completeness` marker moved anywhere, so the `CORPUS_COMPLETE` SET is unmoved too and no seeded fixture was re-dealt) | **PROTOCOL 42 → 43 / HASH 83 → 84 — this cell is EXACTLY right on both halves**, one bump each, both predicted in writing before any production line with both closure type counts predicted and confirmed UNCHANGED at 98 / 132. *(**The row's fix-shape hint is confirmed and was still costed rather than obeyed**: `Target::StackObject` won, and the CR 109.1-literal alternative — registering ability entries in `state.objects` — was measured at stage 0 and REJECTED, because such an entry must claim a `ZoneId` and `casting.rs`'s `TargetSpell` arm decides "is this a spell" by zone ALONE, so a registered ability would have become a legal target for "counter target spell". **One cell of the dispatch brief is REFUTED**: the frontend needed ZERO production lines, because `TargetPicker.svelte` echoes each candidate's `.value` verbatim and never reads `.kind`. **And the batch had to close a THIRD seed to avoid shipping a defect**: making an ability announceable makes it a reachable `ChangeTargets` victim, and `retarget::plan_target_change` derived CR 702.16b protection characteristics from `card_in_stack_zone`, which is `None` for every ability — `OOS-DX25c-3` predicted exactly this and called it "unreachable today". See `memory/primitives/pb-DX52-execution-notes.md`.)* |
| ~~**15**~~ | ~~**PB-DX39**~~ **✅ SHIPPED** (`scutemob-230`, 2026-09-05) *(standing, v3 rank 33)* | source-relative filters through LKI | **OOS-DX5-3** + **OOS-DX5-7** residual — **both CLOSED**, each row corrected against three or four of its own claims | **CORRECTNESS — and "narrow" is a floor by 15×.** The `all_cards()` census puts the resolution-time source-relative class at **41 occ / 29 defs / 21 deck-legal**, of which the three axes this row and the brief name cover 13 occurrences; the rest is a **RACE class no document names** (28 occ / 20 defs / **16 deck-legal**: Craterhoof Behemoth, Mirror Entity, Purphoros, Massacre Wurm, Lathliss, Ezuri, Vito and 13 more). Conversely `mardu_ascendancy` is `partial`, so **OOS-DX5-7's own deck-legal blast radius is ZERO** | **0 flips — EXACTLY right** (coverage unmoved 1,139/1,803 = 63.2% by regeneration; **no `Completeness` marker kind moved anywhere**, checked by `git diff` over the marker rather than inferred, so the `CORPUS_COMPLETE` SET is unmoved and no seeded fixture was re-dealt). `mardu_ascendancy` NOT promoted; its note rewritten to name BOTH blockers | **BOTH UNMOVED — this cell's "none if derivable at resolution" branch is the one that fired, and the reason is CR-correctness rather than cost.** HASH **84** / PROTOCOL **43** gate-executed unmoved, predicted PER OPTION in writing before any production line. *(**The cell's own dichotomy is right and its preference would have been WRONG.** Option (b), a source snapshot stored on the `StackObject` at activation, answers *"the creature equipped when the ability was ACTIVATED"*, while the Jitte's 2005-02-01 ruling says **"most recently equipped"** — so (b) is CR-wrong, not merely a HASH bump. **The seed's preferred option (a) does not work as written either**: SR-24 gates `lki_objects` on four damage keywords and neither subject carries one, so option (a) is *read the LKI AND make it carry the source* — and the two subjects leave at **different moments** (the Jitte with its ability on the stack, Mardu **before** the push), so one gate closes the headline and misses the residual. **The site list was a floor**: 20 source-relative arms and 20 reads, not 17 — the three missed spell the read across a line break and one of them, `AttachedCreature`, IS the headline seed's own arm. **The batch REFUTED two of its own published claims**: the coordinator's predicted zero live-path exposure is **4** emblem registrations (`OOS-DX39-2`), and its published *"the deck-legal live-wrong count is at least TWO"* was withdrawn — `binding_the_old_gods` is deck-legal and live-wrong for a reason ONE LINK UPSTREAM (`fizzle_object`, live-only, makes the whole ability a no-op), which also **refutes PB-DX49's own recorded cause** for that card. So *"repairs all 21 deck-legal defs"* is NOT claimed: a class fix repairs the arithmetic, not every caller's route to it (`OOS-DX39-5`). Tests **5,196 / 0 / 5** (+40, 66 targets, byte-exact set difference: 40 additions / 0 leavers / 0 removals / 0 renames, count-vs-name reconciliation run and duplicate-name scan EMPTY, **re-taken AFTER the `/review` fix cycle**). **The `/review` DEFEATED FIVE OF THE SEVEN SOURCE GATES BY EXECUTION** — `r1` by a map ITERATION rather than an accessor call (its two axes were the same idea, which is `OOS-DX36-8`'s rule broken inside the file that cites it), `r3`+`r4` together by a path-qualified alias with no leading dot, `r3` again by a reader added INSIDE an allowlisted function (PB-DX49's own finding on its own gate, inherited without the fix), `r6b` by a one-line rebinding (PB-DX51's `r1` verbatim) and `r5` by a fn-pointer alias — plus **six wrong CR cites**, including `CR 602.2c`, which DOES NOT EXIST and appears **43 times across 22 files** (40 pre-existing, published for PB-DX38 as `OOS-DX39-10`), and `CR 114.1` quoted for text it does not contain, on which `r7`'s ratchet justification rested. All ten findings taken. Filed **OOS-DX39-1..10**. See `memory/primitives/pb-DX39-execution-notes.md`.)* |
| ~~**16**~~ | ~~**PB-DX53**~~ **✅ SHIPPED** (`scutemob-231`, 2026-09-05) | ~~CR 508.6~~ **the raid gate clobbered across an EXTRA COMBAT** — and **this row's own title mis-cites CR 508.6**, which is verbatim a boolean per-player predicate with no count and no turn content; the `OOS-DX21-1` registry row already said so, and PB-DX21's review had already corrected `legions_landing.rs` for the same mis-cite before it propagated here | **OOS-DX21-1** — **CLOSED**, its row corrected against three of its own claims | **correctness — the "1 deck-legal `Complete`" cell is a FLOOR.** The class is **2**: `minas_tirith` prints *"Activate only if you attacked with two or more creatures this turn"* and was `partial` behind an `ENGINE-BLOCKED` note demanding `Condition::AttackedWithNCreatures(2)` — **an identifier that had existed since PB-OS6 (2026-07-19)**, so the note was false at HEAD. No document in the chain names it | **"0 flips" is REFUTED — ONE flip**, `minas_tirith` `partial` → `Complete`, NAMED in writing before any code (`a37f8239`); coverage 1,139 → **1,140/1,803 = 63.2%** | **"HASH (MED)" is REFUTED — BOTH move: HASH 84 → 85 AND PROTOCOL 43 → 44**, one bump each, both predicted at stage 0 with the reason, closure type counts predicted and confirmed UNCHANGED at 98 / 132. *(**Why the cell was wrong, and it is not a slip**: it assumed the fix is a `PlayerState` field alone, and `PlayerState` really is in `CLOSURE_MUST_NOT_CONTAIN`. But one DSL identifier — `Condition::YouAttackedWithNOrMore` — was read by two cards wanting OPPOSITE CR semantics (CR 508.3d per-declaration for `legions_landing`, ruling 2007-10-01 per-turn-deduplicated for `windbrisk_heights`), so accumulating the field repairs one and REGRESSES the other. The DSL had to split, and `Condition` is on the wire via `Effect::Conditional` — which `rules/protocol.rs`'s own **v21** history row states in as many words, and which was re-verified BY EXECUTION at stage 0 by planting `Condition` in both gates' `CLOSURE_MUST_NOT_CONTAIN`. The PROTOCOL-neutral alternative (evaluate the one variant per-turn as an `activation_condition` and per-declaration inside `Effect::Conditional`) was costed and rejected: it makes one identifier mean two things by POSITION. Both old names were lies and both were renamed — the field said "this turn" and meant "the latest declaration". **This batch's own roster gate then reproduced `OOS-DX36-8` one axis over** (`OOS-DX53-2`): it matched on a `Debug` render, which prints compiled PROSE, so a `Completeness` note quoting `Effect::AdditionalCombatPhase` was counted as a declarer and the population read 5 instead of 4. See `memory/primitives/pb-DX53-execution-notes.md`.)* |
| ~~**17**~~ | ~~**PB-DX54**~~ **✅ SHIPPED** (`scutemob-232`, 2026-09-05) | a resolving spell cannot be its own redirect victim — and **this row's own CR cite is WRONG**, inherited from the seed row and passed on into the dispatch criterion: it says CR 608.2m, which is *"if an instant spell … **leaves the stack once it starts to resolve**, it will continue to resolve fully"*, i.e. an object removed by SOMETHING ELSE mid-resolution. The rule that puts the departure LAST is **CR 608.2n** (`OOS-DX54-1`) | **OOS-DX25c-6** — **CLOSED**, plus rider **OOS-DX25-4** CLOSED and rider **OOS-DX25b-4** DECLINED with a measured cost | **"2 deck-legal `Complete`" REPRODUCES — the first yield cell in five batches that is not a floor**, and the census gives the reason the row does not: `plan_target_change` validates against the VICTIM's requirements, and only the two SINGLE-target ones consult `state.stack_objects`. `TargetSpell` / `TargetSpellWithFilter` / `TargetSpellOrAbility` decide the object branch on `obj.zone == ZoneId::Stack` alone and the resolving spell's CARD never leaves that zone — which is exactly why PB-DX25c's T7 could route around the defect, and why `TargetSpellOrAbility` is a stated CONTROL rather than a third subject. `untimely_malfunction` is a `Partial` third declarer; `deflecting_swat` declares the unaffected variant | **0 flips — EXACTLY right** (coverage unmoved 1,140/1,803 = 63.2% by regeneration, every bucket identical; **0 card-def edits of any kind**, so the shortcut was available and the regeneration was run anyway) | **"HASH (MED)" is REFUTED in the safe direction — BOTH gates UNMOVED, ZERO bumps for the whole PB.** HASH **85** / PROTOCOL **44** gate-executed, closure type counts MEASURED at **98 / 132** by raising each `MIN_CLOSURE_TYPES` to 9999 and reading the gates' own panic. *(**The cell guessed HASH because it assumed the "currently resolving shadow entry" branch of the seed row's own two-option fix shape, and the OTHER option won on a MEASUREMENT the criterion demanded**: with the pop moved to the function boundary the workspace runs **5,207 / 3 / 5** and all three failures are SOURCE gates — **zero behavioural** — so resolve-in-place costs nothing the suite can see and no wire bump, where the shadow entry costs HASH +1 (verified by planting `StackObject` in `hash_schema.rs`'s `CLOSURE_MUST_NOT_CONTAIN`; `protocol_schema.rs` already lists it and `GameState` and stays green) plus a return-type change at six consumers. **AND THAT ZERO IS WHY THE OBVIOUS SHAPE IS WRONG**: removing the entry at the function boundary breaks CR 714.4's Saga sacrifice and CR 309.6's dungeon removal, both of which read `state.stack_objects` from inside `check_and_apply_sbas`, and CR 704.3 checks SBAs AFTER CR 608.2n — no behavioural test in the workspace catches it, which is what "5,207 / 3, zero behavioural" means. Shipped: departure at the two CR-ordered points plus an idempotent backstop for the four early returns that run no trigger/SBA/priority tail. **Two revert rows are COVERAGE MEASUREMENTS rather than passes and say so in the test itself** — R2 (the function-boundary design) and R3 (the backstop) each redden ONE source gate and no behavioural probe; R2's probe is currently UNBUILDABLE behind `OOS-DX54-4` and R3's needs three fixtures nothing in the tree builds (`OOS-DX54-5`). **R6 is the row worth reading**: respelling the departure as `retain(..)` satisfies PB-DX52's inherited `r1a` and is caught only by this batch's `r3`. Tests **5,210 → 5,231 / 0 / 5** (+21, 68 targets, byte-exact set difference: 21 additions / 0 leavers / 0 removals / 0 renames, count-vs-name reconciliation AGREES and the duplicate-name scan is EMPTY on both runs). Filed **OOS-DX54-1..8**, three of them (`-6` gate self-tests, `-7` one-hop gate scope, `-8` the SR-24 LKI-capture cost) by the `/review` fix cycle. See `memory/primitives/pb-DX54-execution-notes.md`.)* |
| ~~**18**~~ | ~~**PB-DX42b**~~ **✅ SHIPPED** (`scutemob-233`, 2026-09-05) | CR 613.1d layer-bounded condition queries | **OOS-ADJ-1** ≡ **OOS-DX19-2** — **FILED *and* CLOSED as ONE defect** (neither had a registry row: six of the adjudication §6's seven seeds were never registered), plus **OOS-DX19-1**'s residue and **OOS-DX19-4** CLOSED BY CONSTRUCTION, and the rank-21 rider **OOS-ADJ-2** TAKEN in both halves | **"7 deck-legal `Complete` pairs" REPRODUCES EXACTLY** — and this batch made it a **CEILING as well as a floor**, by enumerating the five `LayerModification` arms that write `chars.card_types` at the apply site (`Copy` / `SetTypeLine` / `AddCardTypes` / `RemoveCardTypes` / `SetCardTypes`) instead of four remembered variant names; `SetLandTypes` is correctly absent (PB-DX27's `OOS-ADJ-7` repair) and `Copy` has zero corpus supply. The CR 708.2a face-down class is unbounded and is stated, not tallied. The **demand** cell ("2 at HEAD — re-measure") re-measures **correct**; what it does not say is that the second member is `Partial` and reads **Land** where the Archangel reads **Artifact** | **0 flips — EXACTLY right** (coverage unmoved 1,140/1,803 = 63.2% by regeneration, every bucket identical; 3 card-def edits, all comment-only, and `git diff` over the `Completeness::` markers is EMPTY) | **"none (HIGH)" is CORRECT — HASH 85 / PROTOCOL 44 both gate-executed and UNMOVED**, predicted in writing before any production line, closure counts MEASURED at 132 / 98, `git diff` over `hash.rs` and `protocol.rs` EMPTY. *(**The counterfactual was verified by EXECUTION**: planting `TargetFilter`, and separately `EffectLayer`, in each gate's `CLOSURE_MUST_NOT_CONTAIN` fails BOTH gates each time, so STORING the required layer as a field costs +1 on each wire where computing it per instance costs nothing — that measurement is the reason for the design. **The row worth reading is R3.** Deleting the activity sweep's layer bound — the adjudication's OWN §3.2(iii) load-bearing precondition, the one it says is "stated here because it is stated nowhere else" — reddens **NOTHING** in the workspace, and that is structural rather than a missing test: a later-layer effect cannot change an earlier layer's output, which is the very fact that makes bounding semantically free, and the `in_flight` backstop absorbs the rest. It was settled by a **complementary pair**, the first execution of §3.2(iii)'s claim: sweep bound PRESENT + backstop REMOVED runs 23/23 green (termination IS by construction), and sweep bound REMOVED + backstop REMOVED aborts with `fatal runtime error: stack overflow` — `OOS-SIM2-6`'s original crash. R1 and R2 are **precise complements** (R1 the bounded query, R2 the `EffectId` KEYING, which is exactly what a depth counter loses). **BENCHES: a REAL non-overlapping `sba_check` +6.76% regression was FOUND AND REMOVED rather than published** — a per-call `Vec` heap allocation in the bounded layer list, then a per-effect eval-context construction inside an O(permanents × effects) SBA sweep; final 3 base + 5 HEAD runs, every interval OVERLAPS, max +1.51%, nothing claimed. **FUZZ: the 20-game A/B output differs in EXACTLY ONE LINE, the wall clock.** Tests **5,231 → 5,243 / 0 / 5** (+12, 69 targets, delta re-taken AFTER the `/review` fix cycle). Filed **OOS-DX42b-1..7**. `-1` is a debug-build panic this batch shipped and closed; **`-6` is the one a later batch will want**, because it is about every batch's close-out method rather than this one's subject — an end-anchored `... ok|FAILED|ignored$` regex silently drops every `#[ignore = "reason"]` test, so the duplicate-name scan `OOS-DX35-8` mandates was blind to a name it never extracted. **The `/review` found 12 and all 12 were taken; the MEDIUM is the one that matters**: the defect this batch closes was still LIVE one `Condition` variant over, because `YouControlPermanent`/`OpponentControlsPermanent` were fixed at `TypeChange` while both pass their WHOLE filter to `matches_filter` — and the coarse value SILENCED this batch's own `debug_assert`, turning a loud named failure into a silent wrong answer. Both of this batch's new gates were also defeated by execution (a presence check satisfied by a dead closure; a `use … as` alias, which is `OOS-DX54-7` one batch old and not carried across) and both defeats are now RED. See `memory/primitives/pb-DX42b-execution-notes.md`.)* |
| ~~**19**~~ | ~~**PB-DX55**~~ **✅ SHIPPED** (`scutemob-234`, 2026-09-05) | the whole bot/human refusal surface | **OOS-SIM6-3** + **OOS-SIM5-3** + **OOS-SIM5-5** — **all three FILED (none had a registry row) and CLOSED**, plus the rider **OOS-DX51-3** CLOSED | **GATE INTEGRITY / AGENCY — and the shipped alpha's own 422, refuted in the channel it was written about**: a real `POST /api/game/action` drive with an EMPTY pool and no manual `TapForMana`, RED under revert with this cell's own error text | **0 flips — EXACTLY right** (coverage unmoved 1,140/1,803 = 63.2% by regeneration, every bucket identical; **0 card-def edits of any kind**). **THE SPLIT CELL IS WRONG IN EVERY FIGURE AND THE TOTAL IS WRONG TOO.** Re-measured at HEAD by the exact §2.6 invocation before any edit: **70 refusals, not 105** — `OOS-SIM6-3` **18, not 76** (its share falls 72.4% → 25.7% and it is no longer the largest class, so §2.6's own correction of the filed figure to *"76 of 105 — larger in share and in count"* is stale in the other direction); `OOS-SIM5-5` **22, not 2**, GROWN 11× and now the LARGEST class, because PB-DX35's per-mode scoping made the requirement enforceable while leaving the query that satisfies it unchanged; `OOS-SIM5-3` **20, not 27** | **"none (HIGH)" is CORRECT — PROTOCOL 44 / HASH 85 both gate-executed and UNMOVED, ZERO bumps**, predicted in writing per half before any production line (`6a03181a`), closure counts MEASURED at 132 / 98. The cite correction in this cell REPRODUCES (`local_game.rs:1111-1114`) *(**"residue zero" is REFUTED, and so is §2.6's "cast-side refusals are zero of any kind"**: the surface closes at **9**, all of them `expected 1..=1 target(s) but got 0` and **three of them cast-side** — which is `OOS-SIM5-4`'s parked offer-suppression class, priced by §2.6 at 0 of 105 and worth 9 of 9 now that everything else is shut. Filed as `OOS-DX55-1`. **Each half is ONE arithmetic, not a second copy**: `command_mana_cost` is an exhaustive match over all 45 `Command` variants with NO wildcard (so the census is a compiler-enforced CEILING); `check_block_pair`/`validate_block_declaration` are consumed by the handler, by its OWN provoke mirror and by the offer, and `combat.rs` is **+342 / −473, a net reduction of 131 lines**; and `handle_activate_ability`'s INLINE fifth copy of `per_mode_target_requirements` is DELETED. **The engine already held TWO hand-rolled block predicates inside ONE function and they differed in three guards** — the provoke mirror omitted phased-out, `CrossPlayerBlock` and the duplicate check, so an impossible must-block requirement was raised as a refusal instead of skipped. **Three things found only by execution**: the Karn's Bastion self-tap conflict (a mana ability on the permanent about to tap itself); `Umezawa's Jitte`'s modal ability is at layer-resolved index **0**, not the 1 the dispatch brief claimed; and the fuzz gate config's rejection rate is now **0 / 2,717** against a merge base of 5 / 2,713, which forced T2.2's seeds to move and is pinned where it happened rather than left to vanish. **Two CR cites corrected against the rules server**, one of them the plan's own: CR **700.2a** governs modal ACTIVATED abilities (700.2b is the TRIGGERED rule PB-DX35 implements), and `combat.rs:1271` cited CR 509.1c for a rule that is CR **509.1a**. **Revert matrix 13 rows, coordinator-executed** — R1/R2 precise complements, R4's zero settled as structural redundancy by the R4b/R5 pair, **R9 a gate defeat that SUCCEEDED** (all nine markers exotic, so a partial hand-rolled copy scored 1 of 9 and left the gate green; closed by a measured second axis) — **and the harness itself was wrong first**, restoring with `shutil.copy2`, which preserves mtimes, so cargo did not rebuild and every row after the first measured the previous row's binary. Filed **OOS-DX55-1..10**. **The `/review` found 14 and all 14 were taken, and it DEFEATED THREE of the four new source gates by execution** — a binding wildcard, a `use … as` alias taking BOTH axes of the block roster at once, and a struct-pattern destructuring of the per-mode slicer, each invisible to that gate's own self-defeat test because every one of those tests plants the spelling its author already had in mind (`OOS-DX54-6` on three gates at once). All three re-executed against the fixes and RED. It also found the batch's own defect one layer up: the two SET-level blocker guards were HAND-COPIED into `random_bot.rs` instead of calling the exported `validate_block_declaration`, which had zero consumers outside tests; the bot and the browser's 400 boundary both consume it now, and SR-5's keyword registry fired to say the bot had stopped reading Menace. And a THIRD wrong CR cite, going the other way: this batch PROPAGATED `CR 702.110a` for menace into ~10 new lines, and **CR 702.110 is Exploit** — menace is **CR 702.111b**. See `memory/primitives/pb-DX55-execution-notes.md`.)* |
| ~~**20**~~ | ~~**PB-DX56**~~ **✅ SHIPPED** (`scutemob-235`, 2026-09-05) | make the two live fuzz violations diagnosable, then diagnose them | **OOS-FB1-1** *(prerequisite)* — **CLOSED**; **OOS-DX32-1** — **CLOSED**, its central premise CORRECTED; **OOS-DX22-8** — **CLOSED**, diagnosed to a mechanism neither of the two the row told its successor to check first; plus the rider **OOS-DP9-19(b)** CLOSED | **GATE INTEGRITY — achieved: HARD 291 → 0 on the standard invocation, so `--stop-on-error` no longer halts.** Both filed figures were re-measured FIRST and **neither reproduces, both UP**: `player_consistency` 84 → **189** across 4 → 11 of 20 games, `attachment_validity` 22 → **102** across 3 → 7. The cell's "79.2% of HARD" is now **64.9%** | **0 flips — EXACTLY right** (coverage unmoved 1,140/1,803 = 63.2% by regeneration; **0 card-def edits of any kind**, so the shortcut was available and the regeneration was run anyway) | **"none (HIGH)" is CORRECT — HASH 85 / PROTOCOL 44 both gate-executed and UNMOVED, ZERO bumps**, predicted per half in writing before any production line (`1f16e2c9`) with a stated stop-condition if the engine fix had needed a field. *(**`player_consistency` is not one class — it is TWO ARMS THE CR GIVES OPPOSITE DISPOSITIONS, and this row, the seed row and the dispatch criterion all treat them as one.** 189 of 189 reports are the ACTIVE-PLAYER arm and the priority-holder arm measured **zero**. The active arm asserts what **CR 800.4j** *permits* — *"that turn continues to its completion **without an active player**"* — which `TurnState::active_player`, a bare `PlayerId` with ONE write site, cannot express; so it is split TRANSIENT and answered by a strictly stronger **CR 800.4k turn-boundary promotion**, deliberately not an end-state check, because at game end the player who just died may legitimately still be active. The priority arm is a real **CR 800.4a** defect and stays HARD. **Two holes that made the bound lucky rather than true were FIXED**: `advance_turn`'s extra-turn branch applied no liveness filter at all (the one UNBOUNDED route), and `enter_step`'s unconditional cleanup grant (`OOS-DP9-19`, taken as a rider). **`OOS-DX22-8`'s answer is that the check was pointed at the direction that HEALS**: `move_object_to_zone` performs only the CR 702.95e soulbond and replacement-effect fix-ups and touches the attachment relation in NEITHER direction; direction A is cleared by CR 704.5m/704.5n and survives a checkpoint only because nine SBA sweep sites exist and `abilities.rs`/`casting.rs`/`mana.rs`/`combat.rs` contain **zero** — `OOS-M11-7`'s shape one field over. **The engine defect is direction B, which had no check anywhere and never heals**: a host keeps the dead `ObjectId` in `attachments` forever, a field that is HASHED (so it perturbs `public_state_hash` AND CR 104.4b loop detection), read by the CR 510.3a trigger family, walked by CR 702.26g/h phasing through `expect_object_mut` (a latent debug-build panic) and rendered to the browser. **Transient-vs-at-rest settled by an ARITHMETIC the census predicted first**: revert row R-E measures direction B at **10,290 raw / 7 distinct** (~1,470 checkpoints per condition) against direction A's 102 / ~13 (~8) — two orders of magnitude on the same run from the same stateless checker. **CR 400.7f is what makes the fix's one-directionality load-bearing** rather than merely conservative. **The tooling caught a defect in its own consumer on its FIRST run and it was this batch's own** (`OOS-DX56-1`): the CR 800.4k promotion keyed the seat on `player=`, which the prepended state context emits PER SEAT, so it collapsed two different seats into one window and manufactured a false promotion on seed 5 — found by READING the evidence, not the count, which is `OOS-FB1-1`'s entire argument instantiated on the batch that closed it. Tests **5,287 → 5,316 / 0 / 5** (+29), 72 targets, byte-exact NAME set difference with 0 leavers, 0 renames and an EMPTY duplicate scan on both runs, re-taken AFTER the `/review` fix cycle. Merge-base A/B in an isolated worktree differs in **exactly one line, the wall clock**, and every per-class raw count is identical across the boundary, so the engine fixes are trajectory-neutral by measurement. **The adversarial bypass pass defeated SEVEN of eight shipped gates** — including two whose probes called a private function directly, an end-state check with no call-site gate whose hole is INHERITED from PB-DX32, and a promotion with no test of any kind — and all seven are now RED. Filed **OOS-DX56-1..15** (`-6` through `-15` by the `/review` fix cycle, which found 15 and defeated THREE more shipped gates by execution — a commented-out call satisfying a `contains` source gate, an argument swap in the CR 800.4k promotion, and ORing two HARD class names into the transient test, which silently disarms `--stop-on-error`; PB-DX50's "a gate on a DEFINITION says nothing about its CONSUMER" verbatim). See `memory/primitives/pb-DX56-execution-notes.md`.)* |
| ~~**21**~~ | ~~**PB-DX57**~~ **✅ SHIPPED `scutemob-236` 2026-09-05** | the gate-widening cluster — six gates that report success while checking less than they claim | **OOS-DX28-1** + **OOS-DX28-5** + ~~**OOS-ADJ-2**~~ *(the PB-DX42a widening: `t7` 1→8 variants, `t9`'s missing `TargetFilter` half — §2.3)* **✅ TAKEN AS A RIDER BY PB-DX42b (`scutemob-233`, 2026-09-05), both halves, both revert-proven — this row is down one member** + **OOS-DX26-3** + **OOS-DX21-7** + **OOS-DX28-6** *(census first: no gate exists for the "an in-def comment asserts a resolution mechanism the code does not use" shape, and PB-DX27's blocker-note sweep does not cover it; population UNMEASURED, so stage 0 is the census)* | **GATE INTEGRITY** | 0 flips; ~8 lines for the DX42a half alone, which is what makes PB-DX42b's premise actually gated (§2.3) | **none** (HIGH) — test-only | **↻ SHIPPED.** `OOS-DX28-1` CLOSED **as a class**: 35 members enumerated from ~150 classified candidates and PRINTED by a test, 22 unpinned, **20 pinned** (the other 2 are in `tools/`, outside this batch's own 0-engine-lines criterion, and are FILED as `OOS-DX57-3` with their repairs). THREE members were **already stale** when censused and a fourth was found by the repairs. `OOS-DX28-5` CLOSED — **the seed's own instance had already regrown**: PB-DX28 widened the list to six and `Splice.targets` arrived one batch later (PB-DX18), with nothing in the tree able to notice (the compiler cannot, both wire gates exclude the type, and the walk was a literal). `OOS-DX26-3` CLOSED with a 530-row roster pinning the **lowered** order per face across **both** index spaces — the row's own prescription names the wrong list. `OOS-DX21-7` CLOSED: 216 candidate functions read in full, 17 vacuous sites repaired with 19 guard-removal proofs all RED, plus an **18th the sweep missed**, found by the new gate and settled by a complementary pair. `OOS-DX28-6` CLOSED **as a class** with the answer a ZERO — 34 hits, 22 real claims, all TRUE, one comment-only doc-rot repair. `OOS-ADJ-2` verified by execution, not redone. **All gates this batch wrote were then DEFEATED — five in adversarial round 1, five more in round 2, four more by the `/review`, fourteen in all, every one re-keyed and re-executed RED**; the yield cell's *"~8 lines for the DX42a half alone"* is superseded — that half was already shipped by PB-DX42b. Wire **NONE** as predicted, both gates executed unmoved, counterfactual executed. Filed `OOS-DX57-1..5`.
