...
| **OOS-RS3-1** | **Intervening-if is never checked at queue time, only at resolution — CR 603.4 requires both.** `resolution.rs:2125-2135` is the only check; no trigger sweep evaluates `intervening_if` before pushing the `PendingTrigger`. This is a deliberate engine-wide convention, self-documented at the upkeep sweep (`turn_actions.rs:265-266`) — **not** introduced by PB-RS3 and **not** specific to any card. **Divergent case**: the condition is FALSE when the trigger would go on the stack but TRUE by the time it resolves (instant-speed control change, blink, phase-in). Real MTG never triggers; this engine fires. Reachable in 4-player Commander, but narrow. The reverse case (true at queue, false at resolution) is handled correctly. **Scope note**: this affects **every** already-shipped `Complete` def carrying an `intervening_if`, not just PB-RS3's two — `loyal_apprentice` and `siege_gang_lieutenant` were flipped to `Complete` with this limitation recorded explicitly in their completeness notes rather than silently overclaimed. Filed by PB-RS3 (`scutemob-145`), review finding F3. | **correctness, engine-wide** | not ranked (touches every trigger sweep, not just `begin_combat`; a fix must add a queue-time evaluation at each sweep site and decide the shared-helper shape first) |
