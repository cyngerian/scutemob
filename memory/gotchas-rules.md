...
- **Dredge is a replacement effect, not a trigger or activated ability (CR 702.52a).** It
  ...
  player at any time (`draw_card_skipping_dredge`, since deleted, validated nothing). At most
  ONE entry can ever be outstanding per player: a second draw for a player who already owes an
  answer DISCHARGES the earlier one automatically (as though declined,
  `replacement::resolve_declined_pending_draw`) rather than accumulating or stacking a second
  entry. Declining re-checks other `WouldDraw` replacements but does not re-offer dredge for
  THAT SAME draw (`offer_dredge: false`) — this avoids an infinite loop of choices, not a
  "bypassed replacement check".
...
- **The SAME "you"-only scope gap exists on the TRIGGER side (SR-28, trigger-side analog of the PB-Q3 replacement gap above).** `fire_mana_triggered_abilities` (`rules/mana.rs`) scans only permanents where `o.controller == player` (the tapping player) as candidate `WhenTappedForMana` trigger sources. This is correct for the 7 currently-authored `WhenTappedForMana` cards (Mirari's Wake, Zendikar Resurgent, Crypt Ghast, Leyline of Abundance, …), which are all "**Whenever you tap** a [land/creature] for mana" — i.e. only their own controller's taps count. It will **mis-scope** a Vernal Bloom / Mana Flare / Heartbeat of Spring-class card ("Whenever **a player** taps a [Forest/land] for mana, that player adds …" / "**its controller** adds"), which must fire when *any* player taps the matching source. Fixing it needs the same controller-scope field the PB-Q3 replacement fix needs — do both together. Do NOT widen the `o.controller == player` scan without adding that scope discriminator, or the "you"-scoped cards would over-fire for opponents' taps. (SR-28 fixed the orthogonal dead-source bug — `mana_source_matches` and the `AnyLand` replacement filter now read a pre-cost characteristics snapshot per CR 106.12a/b — but deliberately left this scope gap open, matching the PB-Q3 decision.)
