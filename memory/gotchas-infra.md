...
- **`crates/view-model/src/lib.rs` has TWO exhaustive matches** that need updating for every new ability: (1) `StackObjectKind` match in `stack_kind_info()` — add arm for every new SOK variant; (2) `KeywordAbility` match in `format_keyword()` — add arm for every new KW variant. The `ability-impl-runner` misses the `KeywordAbility` match ~50% of the time. Always run `cargo build --workspace` (not just `-p mtg-engine`) after implement/fix phases to catch it — that is still the gate, and it is now a *harder* gate than before, because `mtg-view-model` is a library crate that both `tools/replay-viewer` and (from M11-local S5) `tools/play-server` depend on, so a missed arm breaks the workspace build rather than one binary. Confirmed pattern from Outlast (B9).
  **MOVED 2026-08-01 (M11-local S4, `scutemob-165`)**: both matches used to live in `tools/replay-viewer/src/view_model.rs`, which no longer exists — the file became the new crate's `lib.rs` (a `git mv`, 91% similarity, additive changes only). If a plan or an older doc sends you to the replay-viewer path, it is stale. Nothing else in the workspace matches either enum exhaustively.
