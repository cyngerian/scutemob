...
4,558 (+31)**, delta itemised by test NAME with zero removals. Coverage unmoved at
...
**Three files**: new `crates/engine/tests/core/pb_dx8_oracle_decision_cross_check.rs` (15 tests),
new `crates/engine/tests/core/pb_dx42a_continuous_condition_roster.rs` (10 tests), and
`crates/engine/tests/core/completeness_deviation_scan.rs` rewritten (+906/−34, 4 → 11 tests).
...
Measured: `may` **285** oracle-positive / **72** effectively-`Complete` with no may-shaped
...
118.12 unless-family, `optional: true`, non-null `modes`) — measured 90 → 80 → 72.
