...
| **OOS-DP2-4** | **The seeded-shuffle idiom is copy-pasted at 4 sites** (`effects/mod.rs:3048`, `:3147`, `:8701`, and now `rules/commander.rs`). A `GameState::shuffle_library_seeded(&mut self, player)` on `state/mod.rs` would dedupe it. Deliberately not done in PB-DP2 (plan §4.4): the extraction touches three proven call sites with different error-handling contracts — `move_zone_all_then_shuffle` uses `expect_move_object_to_zone`, which swallows move errors, the opposite of what the mulligan handler needs per MR-M9-12. Batch into a cleanup pass. | cosmetic / refactor | filed by PB-DP2 (`scutemob-150`) |
