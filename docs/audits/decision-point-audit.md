...
| **OOS-DP8-9** | **`handle_concede` advances priority/turn even while ANOTHER player's CR 603.3d announcement is outstanding.** PB-DP8 wires the conceding player's own entry (CR 800.4d drop + CR 800.4j batch resume), but the code after that — "if the conceding player held priority, advance it" and "if it was their turn, advance the turn" — is not gated on `pending_trigger_targets`. So a concede while a *different* controller is mid-announcement can advance the turn under a suspended CR 603.3b batch. **Deliberately left ungated**: the block itself persists (the admission gate still rejects everything but the answer), so this is a CR-ordering wrinkle, not a hang — whereas gating the advance *would* risk a hang, because after the answer nothing would re-drive the abandoned turn. Reachable only when a batch contains real choices for two different controllers and one of them concedes mid-batch. | correctness, narrow | filed by PB-DP8 (`scutemob-156`) |
| **OOS-DP8-10** | **A flush suspended inside `enter_step`'s Cleanup branch skips that branch's `cleanup_sba_rounds` increment and its CR 726 mandatory-loop check.** PB-DP8's resume grants priority through one shared helper (`finish_resumed_flush`) that reproduces the priority half of all five call sites but not `enter_step`'s per-branch bookkeeping. `cleanup_sba_rounds` is a safety valve (max 100) against a pathological cleanup loop, so a cleanup loop that also suspends on a target announcement each round would not ratchet it. Requires a targeted trigger with 2+ candidates firing during cleanup SBAs — vanishingly rare, and the loop-detection fingerprint now hashes `pending_trigger_targets`, which gives CR 104.4b a second grip on the same case. | robustness, narrow | filed by PB-DP8 (`scutemob-156`) |
