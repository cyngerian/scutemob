...
| **OOS-DP7-3** | **`GameEvent::DiscardedToHandSize.reveals_hidden_info()` returns `false` while sibling `CardDiscarded` returns `true`.** Both move a card from a hidden zone to a public one and both name the `ObjectId`. Architecture Invariant 7 (hidden information is enforced by filtering events before broadcast) makes this a **pre-alpha networking correctness** item, not cosmetic — it predates PB-DP7 and was found reading the event's neighbours while adding `CleanupDiscardChoiceRequired`. Check the whole `reveals_hidden_info` match for siblings of the same mistake before fixing just this one. | correctness (hidden info), pre-existing | filed by PB-DP7 (`scutemob-155`) |
