...
|---|---|---|
| **`OOS-ADJ-1`** | MEDIUM | `characteristics_for_condition` returns base characteristics inside the layer walk, deviating from CR 613.1d on **7** measured deck-legal `Complete` pairs — `indomitable_archangel` × { `blinkmoth_nexus`, `inkmoth_nexus`, `darksteel_mutation` } (under-count) and × { `eaten_by_piranhas`, `kenriths_transformation`, `imprisoned_in_the_moon`, `thaumatic_compass` } (over-count) — plus the unbounded CR 708.2a face-down class. **Merges with `OOS-DX19-2`**; both are PB-DX42b. **↻ 2026-09-05 — ✅ CLOSED as ONE defect with `OOS-DX19-2` by PB-DX42b (`scutemob-233`), and REGISTERED: this seed had no row in `docs/audits/decision-point-audit.md` until that batch wrote one — six of this section's seven seeds were never registered, only `OOS-ADJ-7` (by PB-DX27). §5.2's three steps shipped as written, and its §2.5 figure of SEVEN pairs REPRODUCED EXACTLY when re-derived from serialized payloads without consulting this document — and is now a CEILING as well as a floor, because the enumeration came from the five `LayerModification` arms that write `chars.card_types` at the apply site rather than from four variant names. HASH 85 / PROTOCOL 44 gate-executed and UNMOVED, exactly as §5.2 predicted.** |
| **`OOS-ADJ-2`** | MEDIUM | Nothing gates the **size of the corpus population** carrying a layer-querying `ContinuousEffectDef.condition`. It is **1** today, measured. `451e3517`'s `no_condition_evaluator_resolves_characteristics_directly` gates the *evaluator source*, not the population, so a new conditional static passes it and silently joins the deviation. `greymond_avacyns_stalwart`'s rewritten note actively invites one. Wants the §5.1 corpus roster gate; this is the seed that justifies the rank-10 rider. **↻ 2026-09-05 — the v4 rank-21 widening rider was TAKEN by PB-DX42b (`scutemob-233`), and this seed was REGISTERED in the same batch (it had no row until then).** Both halves: `t7` widened from the one-variant `ControlLandWithSubtypes` pin to all EIGHT non-`TargetFilter` layer-querying `Condition` variants, and `t9` gained its missing `TARGET_FILTER_FIELDS`-vs-declaration assertion — the gap `OOS-DX28-1` describes — proven to discriminate by planting a 34th field, which reddens `t9` AND `t6`. The file-header doc's stale "exactly 32 fields" was corrected to 33. **Residual stated**: the gate still measures the population as it EXISTS; seven more ways for it to grow are now pinned. |
| **`OOS-ADJ-3`** | LOW | `OOS-DX19-2` is framed as "a CR 613.8b dependency-aware fixpoint". CR 613.8a(a) confines dependency to a **single layer**; the live case (Layer-6 effect, Layer-4 condition) is strictly cross-layer and needs layer-bounding, not a fixpoint. A worker taking `OOS-DX19-2` at its word will build the wrong thing. Re-word at dispatch. **↻ 2026-09-05 — DISCHARGED**: `OOS-DX19-2`'s registry row re-worded by the coordinator (fixpoint sentence struck, layer-bounded-query fix shape written in with the CR 613.8a(a) reason) immediately before the PB-DX42b dispatch, as the v4 memo §4 row 18 precondition required. |
| **`OOS-ADJ-4`** | LOW | `characteristics_for_condition` is not referentially transparent — the same `(state, obj)` returns different answers by ambient thread-local depth. Any future memoization of `calculate_characteristics` keyed on `(state, id)` (a natural optimization: the Archangel condition performs an O(n) battlefield sweep on **every** query) silently poisons non-layer callers. `451e3517`'s `process_command` balance `debug_assert!` catches a *leaked* depth; it does not catch this, and no comment names the hazard. **↻ 2026-09-05 — the AMBIENT half is gone and the MEMOIZATION half survives, which is the distinction worth keeping.** PB-DX42b (`scutemob-233`) retired the thread-local depth counter entirely, so `characteristics_for_condition`'s answer no longer depends on ambient thread state — that specific referential opacity is closed. What replaces it is an EXPLICIT parameter: `calculate_characteristics_through(state, id, through, eval)` returns different answers for different `through` and `eval`, which is honest rather than hidden, but it means a future memoization keyed on `(state, id)` alone is **still** a hazard — it would have to key on the layer bound and the in-flight set too. Re-scoped to that, and the hazard is now named at the function. |
