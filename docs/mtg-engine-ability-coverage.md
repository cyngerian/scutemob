...
> Last audited: 2026-03-07 (Fuse CR 702.102 validated: was_fused on StackObject, fuse on CastSpell, hand-only validation, combined cost, left-then-right resolution, copy propagation, Wear // Tear, script stack/172, 8 tests in fuse.rs)
...
| Fuse | 702.102 | P4 | `validated` | `state/types.rs`, `state/stack.rs`, `rules/casting.rs`, `rules/resolution.rs`, `rules/copy.rs`, `rules/command.rs`, `cards/card_definition.rs` | Wear // Tear | `stack/172` | — | KW disc 133, AbilDef disc 51; was_fused on StackObject; fuse on CastSpell; hand-only validation; combined cost; left-then-right resolution; copy propagation; 8 unit tests in `fuse.rs` |
...
| Spree | 702.172 | P4 | `validated` | `state/types.rs:L1232`, `rules/casting.rs:L2013-2063`, `cards/card_definition.rs:L1214` | Final Showdown | `stack/173` | — | KW disc 134; mode_costs on ModeSelection; Spree cost block in casting.rs (modes_chosen.sort_unstable() + per-mode cost accumulation); zero-modes validation (CR 702.172a); 9 unit tests in `spree.rs`; game script approved |
...
| Tribute | 702.104 | P4 | `validated` | `state/types.rs` (KW disc 131), `rules/replacement.rs` (inline ETB trigger), `state/game_object.rs` (tribute_was_paid) | Fanatic of Xenagos | `stack/170` | — | KeywordAbility::Tribute(u32) disc 131; tribute_was_paid on GameObject; TributeNotPaid TriggerCondition; opponent choice via inline ETB replacement; bot auto-declines; 8 unit tests in tribute.rs |
| Fabricate | 702.123 | P4 | `validated` | `state/types.rs:L1221` (KW disc 132), `rules/replacement.rs:L971-1020` (inline ETB handler) | Weaponcraft Enthusiast | `stack/171` | — | KeywordAbility::Fabricate(u32) disc 132; inline ETB handler in replacement.rs; bot chooses counters; token fallback (Servo 1/1 colorless artifact creature) if permanent left BF; 9 unit tests in fabricate.rs |
...
| Modal choice | P2 | `validated` | `rules/casting.rs` (validation), `rules/resolution.rs` (mode dispatch), `rules/copy.rs` (propagation) | Abzan Charm | `stack/169` | — | CR 700.2; modes_chosen on CastSpell + StackObject; 14 tests in `tests/modal.rs`; Abzan Charm 3-mode "choose one" instant |
...
All P2 gaps resolved. 17/17 validated.

**Resolved**: Modal choice (CR 700.2) — validated 2026-03-07 (script stack/169, Abzan Charm, 14 unit tests in modal.rs). Crew (CR 702.122) — validated 2026-02-27 (script combat/075, Smuggler's Copter, 15 unit tests in crew.rs).
...
**Resolved**: Fuse (CR 702.102) -- validated 2026-03-07 (script stack/172, Wear // Tear, 8 unit tests in fuse.rs).
