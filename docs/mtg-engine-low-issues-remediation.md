...
| ID | Severity | Description | Location |
|----|----------|-------------|----------|
| PB-N-L01 | LOW | Cosmetic misaligned `filter: None,` indentation blocks in 5 backfilled card defs from the PB-N unit→struct shape change (grim_haruspex.rs:27, cruel_celebrant.rs:25, blood_artist.rs:19, marionette_apprentice.rs, syr_konrad_the_grim.rs). `cargo fmt` does not normalize the misalignment; manual fix only. Cosmetic — no semantic impact. | `crates/engine/src/cards/defs/` (5 files) |
...
| BASELINE-CLIPPY-01 | LOW | `clippy::items_after_test_module` in `tools/replay-viewer/src/main.rs` — top-level fn items defined after a `#[cfg(test)] mod tests { ... }` block. Move items before the test module. | `tools/replay-viewer/src/main.rs` |
| BASELINE-CLIPPY-02 | LOW | `clippy::items_after_test_module` in `tools/replay-viewer/src/replay.rs:368` — multiple fn items after the test module, including `evaluate_assertions`, `check_stack_assertion`, `check_list_assertion`. | `tools/replay-viewer/src/replay.rs:368` |
| BASELINE-CLIPPY-03 | LOW | `clippy::clone_on_copy` in `crates/engine/tests/chosen_creature_type.rs` at lines 364, 466, 471 — `.zone.clone()` on `ZoneId` (which is `Copy`). Replace with `.zone`. | `crates/engine/tests/chosen_creature_type.rs` |
| BASELINE-CLIPPY-04 | LOW | `dead_code` in `crates/engine/tests/scavenge.rs:50` — `fn on_battlefield` is never used. Either use it or delete it. | `crates/engine/tests/scavenge.rs:50` |
| BASELINE-CLIPPY-05 | LOW | `unused_imports` in `crates/engine/tests/graveyard_abilities.rs` — `GameEvent`, `TargetFilter` imported but never referenced. Remove. | `crates/engine/tests/graveyard_abilities.rs` |
| BASELINE-CLIPPY-06 | LOW | `clippy::doc_lazy_continuation` historically observed in a replacement-effects test file (run 2026-04-12 pre-PB-N; not reproduced in the post-PB-N run — cargo's per-target clippy error-bailout order may hide or reveal different warnings between runs). Flagged so the next cleanup pass knows to hunt for it. | `crates/engine/tests/replacement_effects.rs` (approximate) |
...
| ID | Severity | Description | Location |
|----|----------|-------------|----------|
| PB-T-L01 | LOW | Loyalty-ability target validation (`handle_activate_loyalty_ability` in casting.rs) does not use `target_count_range` — it hard-checks `targets.len() != ability.targets.len()` which fails for UpToN slots. No loyalty ability currently uses UpToN (Sorin -6 uses it but is an ETB trigger, not a loyalty activate), but if one is added later it will silently reject legal 0-count casts. Fix: apply `target_count_range` range-gate at loyalty-ability validation site. | `crates/engine/src/rules/casting.rs` loyalty-ability target validation path |
