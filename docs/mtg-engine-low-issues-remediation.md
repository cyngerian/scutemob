...
| ID | Severity | Description | Location |
|----|----------|-------------|----------|
| BASELINE-LKI-01 | LOW (**Status: CLOSED 2026-05-15, LS-7 scutemob-37** `9cbc3c64`) | ~~Filtered death triggers do not match creatures whose filter-relevant characteristic (tested: subtype) was granted while on the battlefield. Reproduced via two independent continuous-effect shapes: `LayerModification::AddSubtypes` through `EffectFilter::SingleObject`, and through `EffectFilter::AttachedCreature` aura grant. **Root cause**: the death-trigger dispatch calls `calculate_characteristics(dying_obj_id)` on the graveyard object, which re-runs the layer system's filter pass; every filter with an `obj_zone == ZoneId::Battlefield` guard drops out (and `SingleObject` fails by identity — CR 400.7 new ObjectId). CR 603.10a + 613.1e require look-back-in-time characteristics for zone-change triggers; engine diverged from CR.~~ Resolved: LKI-completeness audit at `memory/primitives/lki-completeness-audit.md` enumerated all ~25 battlefield-zone-guarded layer filters and all `calculate_characteristics` LKI dispatch sites (exactly one in-scope: `AnyCreatureDies`; LTB/exile/bounce self-triggers structurally immune — they never filter the dying object by characteristic). Fix candidate (a) ruled out — the layer system is non-destructive, so the graveyard object's `characteristics` field holds base/printed values only. Implemented fix candidate (b) as a pre-death snapshot (mirroring the existing `pre_death_power` precedent): `GameEvent::CreatureDied` gains `pre_death_characteristics: Option<Characteristics>`, captured via `calculate_characteristics` **before** `move_object_to_zone` at ~44 construction sites; the `AnyCreatureDies` dispatch evaluates `triggering_creature_filter` against the snapshot. `HASH_SCHEMA_VERSION` bumped 26→27. Regression tests `test_lki_death_filter_subtype_granted_via_single_object` and `test_lki_death_filter_subtype_granted_via_aura` in `crates/engine/tests/pbn_subtype_filtered_triggers.rs` cover both reproduction shapes; PB-N Test 6 de-escalated. 2858 tests passing. (Follow-up LOW noted in the audit: continuous-effect-*granted triggered abilities* are lost the same way via `SelfDies`/`SelfLeavesBattlefield` — out of scope here.) | `rules/abilities.rs` (`AnyCreatureDies` dispatch), `rules/events.rs`, `rules/sba.rs`, `state/hash.rs` |
...
| ID | Severity | Description | Location |
|----|----------|-------------|----------|
| PB-T-L01 | LOW (**Status: CLOSED 2026-05-16, LS-6 scutemob-36** `a2c82c68`, tests `97423497`) | ~~**Loyalty-ability target validation gap**: `handle_activate_loyalty_ability` in `engine.rs` does **not** call `validate_targets` / `validate_targets_with_source` / `validate_object_satisfies_requirement` at any point — it converts `Vec<Target>` to `Vec<SpellTarget>` without any type or filter checks. This means a player can activate a loyalty ability with non-matching targets (e.g. Tyvar Kell +1 with a non-Elf, Basri Ket +1 with a non-creature, Sorin −6 with a non-creature/non-planeswalker) and the effects will run against the wrong game object. **PB-T cards directly affected**: Sorin Lord of Innistrad (−6: UpToN{3, creature/planeswalker}), Basri Ket (+1: UpToN{1, Creature}), Tamiyo Field Researcher (−2: UpToN{2, nonland}), Teferi Temporal Archmage (−1: UpToN{4, Permanent}), Tyvar Jubilant Brawler (+1: UpToN{1, Creature}), Tyvar Kell (+1: UpToN{1, Elf}). The gap is latent for PB-T cards today because integration tests route through the replay harness (not `handle_activate_loyalty_ability`), but would manifest if a player supplied an illegal target via a not-yet-existing simulator or network action. **Fix**: thread the ability's `targets: Vec<TargetRequirement>` through `handle_activate_loyalty_ability` and call `validate_targets_with_source(state, &targets, &ability_targets, player, source_chars, source)` before pushing to the stack — same pattern as `handle_cast_spell`.~~ Resolved: `handle_activate_loyalty_ability` now binds the ability's `targets` and calls `validate_targets_with_source` before paying the loyalty cost (CR 601.2c / 606.4) — illegal targets are rejected and the loyalty counter mutation does not occur. Mirrors `handle_activate_ability`. Tests in `crates/engine/tests/loyalty_target_validation.rs` (6 tests incl. cost-not-paid-on-rejection). | `crates/engine/src/rules/engine.rs` (`handle_activate_loyalty_ability`) |
| PB-T-L02 | LOW (**Status: CLOSED 2026-05-16, LS-6 scutemob-36** engine `a2c82c68`, card def `5a8b404e`, tests `97423497`) | ~~Sorin Lord of Innistrad (-6): the reanimate-rider effect ("return all those creatures to the battlefield under your control") is TODO — annotated `// TODO(PB-T-L02): reanimate-rider`. Blocked on a `MoveZone` variant that can target cards in an opponent's graveyard.~~ Resolved: added `Effect::DestroyAndReanimate { targets, cant_be_regenerated }` (hash disc 85) — destroys the declared targets, then reanimates the resulting non-token graveyard cards under the activating player's control (works across opponents' graveyards). Sorin -6 now uses it; TODO removed. Tests in `crates/engine/tests/destroy_and_reanimate.rs` (6 tests incl. token gate, exile-replacement, indestructible). | `crates/engine/src/cards/defs/sorin_lord_of_innistrad.rs` |
| PB-T-L03 | LOW (**Status: CLOSED 2026-05-16, LS-6 scutemob-36** engine `a2c82c68`, card def `5a8b404e`, tests `97423497`) | ~~Tamiyo Field Researcher (-2): the freeze-rider ("until your next turn") is TODO — annotated `// TODO(PB-T-L03): freeze rider — PreventUntap / UntilControllersNextUntapStep`. Blocked on `EffectDuration::UntilControllersNextUntapStep` and `Effect::PreventUntap` DSL gaps. The tap-targets half is implemented and working.~~ Resolved: "doesn't untap" is an untap-step exception (CR 502.3), not a continuous/layer effect — so the `EffectDuration`/`LayerModification` route was discarded. Added `Effect::PreventNextUntap { target }` (hash disc 86) which increments a new `GameObject.skip_untap_steps: u32` counter; `untap_active_player_permanents` decrements it instead of untapping. Tamiyo -2 now applies it to both targets; TODO removed. Hands of Binding (same freeze rider) also unblocked. Tests in `crates/engine/tests/prevent_next_untap.rs` (5 tests). | `crates/engine/src/cards/defs/tamiyo_field_researcher.rs` |
