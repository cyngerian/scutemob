...
| — | Tilt instead of cargo (build-lock contention between concurrent agents) | worker isolation makes it moot; dispatch hygiene 11 (`/tmp` quota) is the local analogue | No change to the build tool. Its **exit-code contract** — `1` = your code is broken, `3` = I could not find out, and collapsing them teaches callers to distrust the gate — **ADOPTED, `scutemob-257`** into the `/dispatch` worker prompt. |
| — | **manabrew-compat rule 1: classify by payload type, never by concept name** — read the fields of the answering action and the projection, not the mechanic's name | no standing rule; the failure mode is named in memory (`project_legal_but_wrong_gap.md`) as the "legal-but-wrong" risk, which compiles and passes tests | **Yes, cheap. ADOPTED, `scutemob-257`**: `memory/conventions.md` §"Landscape rules" |
| — | **manabrew-compat rule 2: never write "unsupported" without naming the population you searched** — name the enum + variant count, check `git log --grep`, grep-verify the CR number | OOS seeds, `// ENGINE-BLOCKED` notes and `Completeness::partial` blocker text carry unbounded absence claims; three of them outlived the gap they named (§2) | **Yes, cheap. ADOPTED, `scutemob-257`**: same section |
