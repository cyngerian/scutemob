...
plan, because M11-local's close-out was already the task in flight. **All ten HIGH/MEDIUM
findings are closed; all eight LOWs are open**, which is the CLAUDE.md completion
checklist's rule (LOW-only findings need no fix phase and are addressed
opportunistically), not an oversight — each was re-verified as genuinely unchanged at
close-out rather than assumed.
...
| MR-M11-12 | M11-local | `auto_tap_commands_for` cites a `mana_solver.rs` doc sentence that does not exist | OPEN (LOW, deferred) |
...
| LOW (OPEN) | 15 (8 of them M11-local's, per the CLAUDE.md checklist's LOW-no-fix-phase rule; pre-M8 era: MR-M1-18, MR-M6-14 + M8: MR-M8-11 + B16: MR-B16-04, MR-B16-05, MR-B16-06 + PB-AC1: MR-AC1-01 + M11-local: MR-M11-11 through MR-M11-18) |
