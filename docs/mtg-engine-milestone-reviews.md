...
| MR-M11-11 | M11-local | Plan §8 R6's "count only bot passes" mitigation was never implemented | OPEN |
| MR-M11-12 | M11-local | `auto_tap_commands_for` cites a `mana_solver.rs` doc sentence that does not exist | OPEN |
| MR-M11-13 | M11-local | `unreachable!()` in `GameDriver::run_game` — a panic in library code | OPEN |
| MR-M11-14 | M11-local | Rewritten `check_stack_consistency` no longer rejects duplicate `source_object` claims | OPEN |
| MR-M11-15 | M11-local | Auto-tap atomicity asymmetry between `advance()` and `submit` is undocumented | OPEN |
| MR-M11-16 | M11-local | `UptoN` / per-mode / `needs_x`-on-`CastSpell` paths ship unexercised and untracked | OPEN |
| MR-M11-17 | M11-local | `event_kind` serializes the whole event per call; catch-all `text` is a bare variant name | OPEN |
| MR-M11-18 | M11-local | `redact_stack` O(n²) and `legal_targets_per_slot` candidate sweep — measured once, unbounded | OPEN |
...
| MR-M11-21 | M11-local | Wire-neutrality held end to end — PROTOCOL 32 / HASH 69 unmoved, one 227-line read-only engine module | -- |
...
| CRITICAL | 0 |
| HIGH (OPEN) | 1 (M11-local: MR-M11-01) |
...
| MEDIUM (OPEN) | 11 (pre-M8: MR-M7-09, MR-M7-12 -- deferred to M10+ ; M11-local: MR-M11-02 through MR-M11-10) |
...
| LOW (OPEN) | 15 (pre-M8 era: MR-M1-18, MR-M6-14 + M8: MR-M8-11 + B16: MR-B16-04, MR-B16-05, MR-B16-06 + PB-AC1: MR-AC1-01 + M11-local: MR-M11-11 through MR-M11-18) |
...
+ ~4,300 Rust test + ~2,867 Svelte/JS/CSS/config + 782 README. Wire surface unmoved (PROTOCOL 32 / HASH 69).
