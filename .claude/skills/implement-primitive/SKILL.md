...
This skill was rewired off the retired `docs/project-status.md` and the HISTORICAL
`docs/primitive-card-plan.md` (DOCB-2, `scutemob-132`). Everywhere below, "the active
queue-plan file" means:

- **Active queue-plan file** — the ranked PB queue + per-batch specs + status/backlog table.
  This is a dated file under `memory/primitives/`; the current one is named in the
  **CLAUDE.md "Current State"** section (as of 2026-07-18:
  `memory/primitives/oos-retriage-plan-2026-07-18.md`, whose §3 "Ordered PB queue" holds the
  batch specs and whose "Queue summary" table is the status/backlog). Always resolve the
  filename from CLAUDE.md Current State — do not hard-code a stale one.
- **Card health / coverage** — `docs/authoring-status.md`, regenerated by
  `python3 tools/authoring-report.py` (canonical, self-dating). Never `docs/project-status.md`
  (RETIRED) — it has no successor generator.
- **Milestone / queue / test-count snapshot** — the **"Current State"** section of `CLAUDE.md`.
- **Handoffs & workstream status** — `memory/workstream-state.md`.
...
5. `Close PB-<N>` -- "Update the active queue-plan status/banner, workstream-state.md, CLAUDE.md"
...
3. `Close PB-<N> review` -- "Update the active queue-plan status/backlog, workstream-state.md"
...
- Read the active queue-plan file's status/backlog table (its "Queue summary" table).
- If there are batches still needing a retroactive review (`pending`), suggest the next one:
  > **W6-review is the primary objective.** Next pending review: PB-<N> (<cards> cards).
  > Run `/implement-primitive --review-only PB-<N>` to start.
- If all reviews are complete, find the next unshipped PB batch from the queue (the first row
  not struck/✅ SHIPPED, in ranked order).
...
3. **Update CLAUDE.md Current State** (inline -- no agent needed):
   ...
   - Update the active-queue bullet so the leading Current State line matches (don't
     leave line ~17 stale against the queue — audit #2 N1)
