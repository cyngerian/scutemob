...
fn render_summary(f: &mut Frame, area: Rect, app: &App) {
    ...
    let total = cc.total();
    let filter_note = if app.show_gaps_only { "  [g: showing gaps only]" } else { "  [g: show all]" };

    let text = format!(
        "  Covered: {} ({:.0}%)  Gap: {} ({:.0}%)  Deferred: {} ({:.0}%)  Partial: {}{}",
        cc.covered,
        if total > 0 { cc.covered as f64 / total as f64 * 100.0 } else { 0.0 },
        cc.gap,
        if total > 0 { cc.gap as f64 / total as f64 * 100.0 } else { 0.0 },
        cc.deferred,
        if total > 0 { cc.deferred as f64 / total as f64 * 100.0 } else { 0.0 },
        cc.partial,
        filter_note,
    );
...
fn render_table(f: &mut Frame, area: Rect, app: &mut App) {
    let cases: Vec<&crate::dashboard::data::CornerCase> = if app.show_gaps_only {
        app.data.corner_cases.cases.iter().filter(|c| c.status == "GAP").collect()
    } else {
    ...
    let rows: Vec<Row> = cases.iter().map(|c| {
        ...
        let milestone_text = if c.milestone.is_empty() { "—".to_string() } else { c.milestone.clone() };

        Row::new(vec![
            Cell::from(c.number.to_string()),
            Cell::from(c.name.clone()),
            Cell::from(Span::styled(c.cr_refs.clone(), Style::default().fg(Color::DarkGray))),
