...
fn main_loop(terminal: &mut Terminal<CrosstermBackend<io::Stdout>>, app: &mut App) -> Result<()> {
    ...
    let reload_interval = Duration::from_secs(120);
...
fn handle_key(app: &mut App, key: crossterm::event::KeyEvent) {
    ...
    match key.code {
        ...
        KeyCode::Down | KeyCode::Char('j') => match app.current_tab {
            1 => app.pipeline_scroll_down(),
            2 => app.cards_scroll_down(),
            3 => app.milestone_scroll_down(),
            _ => {}
        },
        KeyCode::Up | KeyCode::Char('k') => match app.current_tab {
            1 => app.pipeline_scroll_up(),
            2 => app.cards_scroll_up(),
            3 => app.milestone_scroll_up(),
            _ => {}
        },
        KeyCode::Char('J') => {
        ...
        KeyCode::Char('t') => {
            if app.current_tab == 2 {
                app.set_cards_filter("todo");
            }
        }
        KeyCode::Char('o') => {
            if app.current_tab == 2 {
                app.set_cards_filter("ok");
            }
        }
        KeyCode::Char('p') => {
            if app.current_tab == 2 {
                app.set_cards_filter("partial");
            }
        }
        KeyCode::Char('s') => {
            if app.current_tab == 2 {
                app.set_cards_filter("stripped");
            }
        }
        KeyCode::Char('a') => {
            if app.current_tab == 2 {
                app.set_cards_filter("all");
            }
        }
        _ => {}
    }
