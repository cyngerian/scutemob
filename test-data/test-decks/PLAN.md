...
## Phase 5: Ability-Gated Worklist — DONE
...
## Phase 6: Refactor Card Definitions Structure — DONE

### Problem

`definitions.rs` was 3,118 lines with all 112 cards in one `vec![]` literal. At
500+ cards this became unworkable: agent insertion failures, slow compilation,
merge conflicts between parallel authoring sessions.

### Solution (as implemented)

Keep cards inside `crates/engine/` (not a separate crate) with one file per card
and `build.rs` auto-discovery. This avoids circular dependency issues.

```
crates/engine/
├── build.rs                          (scans defs/, generates mod + collector)
└── src/cards/
    ├── card_definition.rs            (UNCHANGED)
    ├── registry.rs                   (UNCHANGED)
    ├── mod.rs                        (helpers + defs modules)
    ├── helpers.rs                    (DSL prelude — all types + helper fns)
    └── defs/
        ├── mod.rs                    (include! generated code)
        ├── sol_ring.rs               (pub fn card() -> CardDefinition)
        ├── arcane_signet.rs
        └── ...112 files
```
...
## Phase 7: Fix the Card-Definition-Author Agent — DONE
...
## Phase 8: Scryfall Skeleton Generator — DONE
...
### Script: `tools/generate_skeleton.py`
...
pub fn card() -> CardDefinition {
    CardDefinition {
        ...
        abilities: vec![
            ...
            // TODO: Triggered — Whenever equipped creature dies, draw two cards.
            // TODO: Equip — Equip {1}
        ],
        ..Default::default()
    }
...
Uses local deck data (1,174 cards indexed) with Scryfall API fallback.
This pre-populates files. The agent or human then only needs to fill in `abilities`.

---

## Phase 9: Batch Authoring — READY
...
| 2 | Phase 5: Build ability-gated worklist | DONE | Phase 4 data |
| 3 | Phase 6: Refactor definitions structure | DONE | — |
| 4 | Phase 7: Fix card-definition-author agent | DONE | Phase 6 (new file structure) |
| 5 | Phase 8: Scryfall skeleton generator | DONE | Phase 6 (target directory) |
| 6 | Phase 9: Batch authoring begins | READY | Phases 5-8 |

All infrastructure is in place. Phase 9 batch authoring can begin.
