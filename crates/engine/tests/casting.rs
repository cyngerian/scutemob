...
fn test_cast_spell_sorcery_speed_happy_path() {
    ...
    assert!(events.iter().any(|e| matches!(e, GameEvent::SpellCast { player, .. } if *player == p1)));
    assert!(events.iter().any(|e| matches!(e, GameEvent::PriorityGiven { player } if *player == p1)));
