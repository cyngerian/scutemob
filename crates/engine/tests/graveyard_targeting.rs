...
fn pass_all_four(state: GameState, turn_order: [PlayerId; 4]) -> (GameState, Vec<GameEvent>) {
    let mut s = state;
...
fn test_115_1_target_creature_in_your_graveyard_valid() {
    ...
    let dead_creature = ObjectSpec::creature(p1, "Dead Bear", 2, 2).in_zone(ZoneId::Graveyard(p1));
...
fn test_115_1_resolve_return_creature_from_gy_to_battlefield() {
    ...
    assert_eq!(
        bf_bears.len(),
        1,
        "Dead Bear should be on the battlefield after resolve"
    );
    ...
    let gy_names: Vec<_> = state
        ...
        .collect();
    assert!(
        gy_names.contains(&"Raise Dead"),
        "Sorcery should be in graveyard"
    );
    assert!(
        !gy_names.contains(&"Dead Bear"),
        "Dead Bear should not be in graveyard"
    );
}
...
fn test_115_1_your_gy_rejects_opponent_gy_card() {
    ...
    assert!(
        result.is_err(),
        "TargetCardInYourGraveyard should reject opponent's graveyard"
    );
}
...
fn test_115_1_any_gy_accepts_opponent_gy_card() {
    ...
    assert!(
        result.is_ok(),
        "TargetCardInGraveyard should accept any player's graveyard"
    );
}
...
fn test_115_1_filter_rejects_wrong_card_type() {
    ...
    let result = process_command(
        state,
        default_cast(p1, spell_id, vec![Target::Object(artifact_id)]),
    );
    assert!(
        result.is_err(),
        "creature filter should reject artifact cards in graveyard"
    );
}
...
fn test_115_1_gy_targeting_rejects_battlefield_card() {
    ...
    assert!(
        result.is_err(),
        "graveyard targeting should reject battlefield cards"
    );
}
...
fn test_has_subtypes_or_filter_matches() {
    ...
    let result = process_command(
        state,
        default_cast(p1, spell_id, vec![Target::Object(wizard_id)]),
    );
    assert!(
        result.is_ok(),
        "Wizard should match 'Vampire or Wizard' OR filter"
    );
}
...
fn test_has_subtypes_or_filter_rejects_no_match() {
    ...
    let result = process_command(
        state,
        default_cast(p1, spell_id, vec![Target::Object(goblin_id)]),
    );
    assert!(
        result.is_err(),
        "Goblin should be rejected by 'Vampire or Wizard' filter"
    );
}
...
fn test_115_1_return_artifact_to_hand() {
    ...
    let hand_names: Vec<_> = state
        ...
        .collect();
    assert!(
        hand_names.contains(&"Sol Ring"),
        "Sol Ring should be returned to hand: got {:?}",
        hand_names
    );
}
...
fn test_608_2b_fizzle_gy_target_exiled_before_resolution() {
    ...
    let (state, _) = process_command(
        state,
        default_cast(p1, raise_id, vec![Target::Object(creature_id)]),
    )
    .unwrap();
