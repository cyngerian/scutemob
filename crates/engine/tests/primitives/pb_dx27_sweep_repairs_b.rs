...
/// PB-DX45: `Effect::MayPayThenEffect` (Vampire Gourmand's "you may sacrifice a
/// creature", Ruthless Technomancer's paired instance) now asks
/// `EffectChoiceQuestion::PayOptionalCost` on the CR 608.2d suspend-and-replay
/// channel instead of auto-paying, so a resolution that hits it leaves
/// `pending_effect_choice` set and the next `PassPriority` is rejected with
/// `BlockedByPendingDecision`. Drain it through `auto_answer_blocking_decisions`
/// after each pass; its default answer for `PayOptionalCost` is `pay: true`,
/// byte-identical to the pre-PB-DX45 unconditional pay these tests are pinning.
