...
// ── Test 8: No Bargain keyword → sacrifice ignored (RC-1 consolidation) ───────

/// RC-1 consolidation: providing Sacrifice in additional_costs for a spell without
/// the Bargain keyword is silently ignored. The sacrifice is not performed and the
/// spell casts normally. (Previously this was an error, but after consolidation the
/// Sacrifice entry is shared across bargain/casualty/devour/emerge.)
#[test]
fn test_bargain_no_keyword_sacrifice_ignored() {
    let p1 = p(1);
    let p2 = p(2);
    ...
    // Providing Sacrifice in additional_costs without any matching keyword.
    ...
    assert!(
        result.is_ok(),
        "RC-1: unclaimed sacrifice should be silently ignored; got: {:?}",
        result.err()
    );
}
