...
fn test_aftermath_exile_on_counter() {
    ...
    // all-pass round: Counterspell resolves, countering (and exiling, per
    // aftermath) Ribbons in the same resolution -- the stack is now empty, so
    // CR 117.3b hands priority to the active player p1 for the next round; the
