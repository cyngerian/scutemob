...
// ── SR-30: re-attach re-timestamps the equipment's continuous effects ──────────

/// CR 613.7a + CR 613.7e (SR-30): when an Equipment becomes attached it receives a
/// new timestamp, and *each continuous effect generated by its static abilities
/// receives a new timestamp as well*. Layer ordering reads
/// `ContinuousEffect.timestamp`, so a re-equipped Equipment must now win a
/// same-layer (Layer 7b P/T-set) conflict it previously lost by timestamp.
///
/// Scenario (a real layer-order flip):
///   - Equipment A sets equipped creature to 6/6, attached to X first (older).
///   - Equipment B sets equipped creature to 4/4, attached to X second (newer).
///     B wins by timestamp → X is 4/4.
///   - A is re-equipped away to Y and back to X. Its final attach to X gives A —
///     and A's continuous effect — the newest timestamp, so A now wins → X is 6/6.
///
/// Before SR-30, the attach sites bumped only the equipment object's timestamp
/// (which feeds the hash, not layer ordering), leaving A's continuous effect
/// frozen at its registration timestamp; B kept winning and X stayed 4/4 — this
/// assertion would fail.
#[test]
fn test_reequip_retimestamps_continuous_effects_layer_order_flip() {
    let p1 = p(1);
    let p2 = p(2);

    let sword_a = mtg_engine::ObjectSpec::artifact(p1, "Sword A")
        .with_subtypes(vec![SubType("Equipment".to_string())])
        .with_activated_ability(equip_ability(0));
    let sword_b = mtg_engine::ObjectSpec::artifact(p1, "Sword B")
        .with_subtypes(vec![SubType("Equipment".to_string())])
        .with_activated_ability(equip_ability(0));
    let creature_x = mtg_engine::ObjectSpec::creature(p1, "Creature X", 1, 1);
    let creature_y = mtg_engine::ObjectSpec::creature(p1, "Creature Y", 1, 1);
    ...
    let a_id = find_object(&state, "Sword A");
    let b_id = find_object(&state, "Sword B");
    let x_id = find_object(&state, "Creature X");
    let y_id = find_object(&state, "Creature Y");

    // Attach both swords to X and register their static P/T-set effects with
    // controlled timestamps: A older, B newer. (These equipment objects are not
    // real card defs, so we inject the continuous effects the ETB path would.)
    for (equip, target) in [(a_id, x_id), (b_id, x_id)] {
        state.objects_mut().get_mut(&equip).unwrap().attached_to = Some(target);
        state
            .objects_mut()
            .get_mut(&target)
            .unwrap()
    ...
    let base = state.current_timestamp();
    let ts_a = base + 1; // older
    let ts_b = base + 2; // newer → wins initially
    *state.timestamp_counter_mut() = ts_b;
    state.continuous_effects_mut().push_back(ContinuousEffect {
        id: EffectId(9001),
        source: Some(a_id),
        timestamp: ts_a,
    ...
    // Initially B (newer timestamp) wins the Layer 7b set → X is 4/4.
    let chars = calculate_characteristics(&state, x_id).unwrap();
    assert_eq!(
        (chars.power, chars.toughness),
        (Some(4), Some(4)),
        "before re-equip, B's newer SetPowerToughness should win → 4/4"
    );

    state.turn_mut().priority_holder = Some(p1);

    // Re-equip A away to Y, then back to X. Each attach re-timestamps A's
    // continuous effect (CR 613.7a); after the final attach A is newest.
    let equip_to = |state: mtg_engine::GameState, target: ObjectId| {
        let (state, _) = process_command(
            state,
            Command::ActivateAbility {
                player: p1,
        ...
        .unwrap();
        pass_all(state, &[p1, p2]).0
    };
    let state = equip_to(state, y_id);
    let state = equip_to(state, x_id);

    assert_eq!(
        state.objects().get(&a_id).unwrap().attached_to,
        Some(x_id),
        "Sword A should be back on Creature X"
    );

    // After the re-equip, A's continuous effect has the newest timestamp → A wins.
    let chars = calculate_characteristics(&state, x_id).unwrap();
    assert_eq!(
        (chars.power, chars.toughness),
        (Some(6), Some(6)),
        "after re-equip, A's re-timestamped SetPowerToughness must win → 6/6 (CR 613.7a)"
    );
}
