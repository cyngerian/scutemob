...
///
/// One deliberate consequence: this reads FULLY layer-resolved characteristics
...
/// `chars` resolved only through the layers *before* the current one. Every
/// mass-filter def in the PB-DX5 roster creates its effect in Layer 6 or 7 and
/// reads type/subtype (settled by Layer 4) or color (settled by Layer 5), so
/// this can only diverge for a resolution-generated effect in Layer <= 4 whose
/// filter reads a type a later-timestamped Layer-4 effect changes -- none exist
/// in the roster (verified: `pb_dx5_continuous_effect_roster` step 7.4). Full
/// resolution is the CR-correct input either way (CR 611.2c determines the set
/// once, from the state as it stands when the effect begins).
///
/// Only `Effect::ApplyContinuousEffect` populates `affected_set`. Every other
/// creation site (`rules/resolution.rs`'s 13 keyword-trigger grants,
/// `rules/replacement.rs`'s static-ability registrations, `effects/mod.rs`'s
/// token P/T overrides, `Effect::CreateEmblem`'s static-effect loop,
/// `rules/copy.rs`'s Layer-1 copy effect) is either a static ability (CR
/// 611.3a: not locked in) or a `SingleObject` filter (locking is a no-op). A
/// future mass filter at any other resolution-time site would be a silent
/// CR 611.2c hole -- filed as OOS-DX5-1.
