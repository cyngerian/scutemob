...
pub use mtg_card_types::state::{
    ...
    TriggerDoublerFilter, TriggerEvent, TriggerTargetOption, TriggeredAbilityDef,
    TurnFaceUpMethod, UpkeepCostKind,
...
impl GameState {
    ...
    ///
    /// CR 613.7a: "If ... the object the ability is on receives a new timestamp,
    ...
    /// timestamps remains the same."
    ///
    /// Before SR-30, the attach sites bumped only `obj.timestamp` (which feeds
    /// the state hash but not layer ordering), leaving the `ContinuousEffect`
    /// timestamps frozen at registration time. A re-attached equipment therefore
    /// could never win a same-layer conflict it should have won by its newer
    /// timestamp. This method bumps both, so layer ordering (which reads
    /// `ContinuousEffect.timestamp`) follows the re-attach.
    ///
    /// The object receives the earlier fresh timestamp and its continuous
    /// effects the later ones, matching the ETB registration order (the object's
    /// own timestamp precedes the static-ability effects it generates —
    /// CR 613.7n). The effects' *relative* order is preserved.
    ///
    /// Call this at every (re-)attach site in place of a bare
    /// `timestamp_counter += 1; obj.timestamp = ...`.
    pub(crate) fn retimestamp_attached_source(&mut self, source_id: ObjectId) {
        ...
        // then reassign fresh sequential timestamps to preserve relative order.
        let mut affected: Vec<(EffectId, u64)> = self
            .continuous_effects
            .iter()
            .filter(|e| e.source == Some(source_id))
            .map(|e| (e.id, e.timestamp))
            .collect();
        affected.sort_by_key(|(_, ts)| *ts);
        for (eid, _) in affected {
            self.timestamp_counter += 1;
            let new_ts = self.timestamp_counter;
            if let Some(e) = self.continuous_effects.iter_mut().find(|e| e.id == eid) {
                e.timestamp = new_ts;
            }
        }
    }
