...
fn execute_effect_inner(
    ...
    match effect {
        ...
        Effect::ApplyContinuousEffect { effect_def } => {
            ...
            // require the deferred Layer-7c dynamic-static primitive (PB-CC-C-followup).
            // `CdaPowerToughness` (Layer 7a, dispatched via `resolve_cda_amount` in layers.rs)
            // is the closest existing dynamic-static analogue.
