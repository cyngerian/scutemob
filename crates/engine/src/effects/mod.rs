...
fn execute_effect_inner(
    ...
    match effect {
        ...
        // CR 400.7 / 712.18 / PB-OS4 (OOS-EF5-3): return ctx.source (already off the
        // battlefield, e.g. a graveyard object from a WhenDies trigger) to the
        // battlefield already transformed, IMMEDIATELY -- no exile step, no delay.
        // Same shape as Persist/Undying's Effect::MoveZone { Source -> Battlefield },
        // except the returned object also flips to its back face and registers
        // statics/ETB triggers (mirrors the delayed dispatch arm in resolution.rs).
        Effect::ReturnSourceToBattlefieldTransformed => {
            let source_id = ctx.source;
            let controller = ctx.controller;
            let owner_and_card = state
                .fizzle_object(source_id)
                .map(|o| (o.owner, o.card_id.clone()));
            let Some((owner, card_id_opt)) = owner_and_card else {
                return;
            };
            // CR ruling: if the card isn't a double-faced card, it stays put.
            let is_dfc = card_id_opt
                .as_ref()
                .and_then(|cid| {
                    state
            ...
            let Some((battlefield_id, _)) =
                state.fizzle_move_object_to_zone(source_id, ZoneId::Battlefield)
            else {
                return;
            };
            let ts = state.timestamp_counter;
