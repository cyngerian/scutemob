...
pub enum AbilityDefinition {
    ...
    /// Static ability that **modifies** (Layer 7c) power and/or toughness dynamically,
    /// re-evaluating at every `calculate_characteristics` call (CR 611.3a).
    ///
    /// ## Contrast with related primitives
    ///
    /// - **`CdaPowerToughness` (Layer 7a, disc 70)**: *sets* base P/T via `SetPtDynamic`.
    ///   Use for `"This creature's power and toughness are each equal to N"` patterns.
    ///   A Layer-7b "becomes a 0/2" effect will override that set-value; `CdaModifyPowerToughness`
    ///   is applied after Layer 7b (CR 613.4c), so it correctly stacks on top of any base value.
    ///
    /// - **`LayerModification::ModifyPowerDynamic` via `Effect::ApplyContinuousEffect`
    ///   (CR 608.2h)**: *locks the value at spell/ability resolution time*. The dynamic
    ///   variant is substituted to a concrete integer at `execute_effect` time so X is
    ///   frozen (e.g. "creatures get -X/-X where X is the number of Vampires you control"
    ///   — Olivia's Wrath). Do NOT route a continuously-updating static ability through that
    ///   path; the `debug_assert!` guard in `apply_modification` will fire.
    ///
    /// ## This variant (CR 611.3a)
    ///
    /// `register_static_continuous_effects` stores a `ContinuousEffect` with
    /// `modification: ModifyPowerDynamic { amount, negate: false }` (or `ModifyToughnessDynamic`
    /// / `ModifyBothDynamic` depending on which fields are `Some`), `is_cda: true`, and
    /// `condition: None` (CR 604.3a(5)).
    ///
    /// `apply_layer_modification` then calls `resolve_cda_amount` live on every
    /// `calculate_characteristics` invocation — the value is **never** locked in.
    ///
    /// ## Authoring rules
    ///
    ...
    /// CR 613.4c: Layer 7c — effects and counters that **modify** (not set) P/T.
    /// CR 604.3a(5): CDAs have no conditions.
    ///
    /// ## Cards using this variant
    ///
    /// - Vishgraz, the Doomhive — `+1/+1 for each poison counter your opponents have`
    ///   (`ModifyBothDynamic`, `PlayerCounterCount { player: EachOpponent, counter: Poison }`).
    /// - Exuberant Fuseling — `+1/+0 for each oil counter on it`
    ///   (`ModifyPowerDynamic`, `CounterCount { target: Source, counter: Oil }`).
    ///
    /// Introduced in PB-CC-C-followup (2026-04-29).
    ///
    /// Discriminant 76.
    CdaModifyPowerToughness {
        /// Amount to add to power per "unit" of the dynamic quantity. `None` = no power modification.
        power: Option<EffectAmount>,
        /// Amount to add to toughness per "unit" of the dynamic quantity. `None` = no toughness modification.
        toughness: Option<EffectAmount>,
    },
