...
pub fn card() -> CardDefinition {
    CardDefinition {
        ...
        oracle_text: "As an additional cost to cast this spell, you may collect evidence 6. \
                      (Exile cards with total mana value 6 or greater from your \
                      graveyard.)\nFlying\nWhen this creature enters, tap target creature. If \
                      evidence was collected, put a stun counter on it. (If a permanent with a \
        ...
        completeness: Completeness::partial(
            "CounterType::Stun exists, but no engine handling of stun counters (CR 701.59: \
             replace untap by removing a counter) — grep 'Stun' in rules/ finds nothing. Adding \
             the counter would be a silent no-op, so the if_true branch is left empty per W5. \
             Unblock: implement the stun-counter untap replacement, then wire Effect::AddCounter \
             { counter: CounterType::Stun, count: 1 }.",
        ),
    }
}
