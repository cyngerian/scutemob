...
pub fn card() -> CardDefinition {
    CardDefinition {
        ...
        completeness: Completeness::partial(
            "Blocked on: (a) 'you may tap or untap target permanent' — Effect::Choose is \
             non-interactive (effects/mod.rs:3190 always executes the first option), so the modal \
             choice cannot be expressed; (b) '{1}{G}{W}{U}: Put Derevi onto the battlefield from \
             the command zone' — no activated-from-command-zone mechanism. NOTE: both trigger \
             conditions DO exist (WhenEntersBattlefield; \
             WheneverCreatureYouControlDealsCombatDamageToPlayer, chain replay_harness.rs:3020 -> \
