...
pub fn card() -> CardDefinition {
    CardDefinition {
        ...
        oracle_text: "Cumulative upkeep {1}{B} (At the beginning of your upkeep, put an age \
                      counter on this permanent, then sacrifice it unless you pay its upkeep cost \
                      for each age counter on it.)\nAt the beginning of each upkeep, if this \
                      enchantment is on the battlefield, each player creates a 2/2 black Zombie \
                      creature token with haste named Tombspawn for each creature card in their \
                      graveyard.\nAt the beginning of each end step and when this enchantment \
                      leaves the battlefield, destroy all tokens created with this enchantment. \
        ...
        completeness: Completeness::partial(
            "Blocked on token-origin tracking — 'destroy all tokens created with this \
             enchantment' requires per-token provenance the engine does not record — and on a \
             combined 'each end step AND when this leaves the battlefield' trigger. NOT a gap: \
             TriggerCondition::AtBeginningOfEachUpkeep exists and is dispatched. Per-player 'a \
             token for each creature card in THEIR graveyard' needs verification \
             (ForEach::EachPlayer + EffectAmount::CardCount).",
        ),
        ..Default::default()
    }
}
