...
pub fn card() -> CardDefinition {
    CardDefinition {
        ...
        oracle_text: "Flying\nWhenever you cast a spell, return up to one target nonland \
                      permanent an opponent controls with mana value less than or equal to that \
                      spell's mana value to its owner's hand."
            .to_string(),
        power: Some(6),
        toughness: Some(6),
        abilities: vec![
        ...
        completeness: Completeness::partial(
            "DSL gap — targeted trigger with a dynamic MV comparison filter (≤ spell's MV) is not \
             expressible; TargetFilter has no...",
        ),
        ..Default::default()
    }
}
