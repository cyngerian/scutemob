...
pub fn card() -> CardDefinition {
    CardDefinition {
        ...
        oracle_text: "Return target creature card from your graveyard to the battlefield. If it's \
                      an Elf, create two 1/1 green Elf Warrior creature tokens.\nForetell {3}{B}"
            .to_string(),
        abilities: vec![
        ...
        completeness: Completeness::partial(
            "Return creature from graveyard + conditional Elf check for token creation — \
             subtype-conditional effect branch not in DSL",
        ),
        ..Default::default()
    }
