...
pub fn card() -> CardDefinition {
    CardDefinition {
        ...
        completeness: Completeness::inert("Blocked on commander-gated modal widening: ModeSelection has fixed min_modes/max_modes and cannot express 'if you control a commander as you cast this spell, you may choose both instead' (CR 700.2). The two mode bodies themselves are expressible (ApplyContinuousEffect + AddKeywords over CreaturesYouControl, UntilEndOfTurn); 'protection from each color' needs a protection-quality grant check. Unblock with a condition-gated max_modes on ModeSelection. Shares this blocker with jeskas_will.rs."),
        ..Default::default()
    }
}
