...
pub fn card() -> CardDefinition {
    CardDefinition {
        ...
        completeness: Completeness::partial("Activated ability blocked on two primitives: (1) Cost has no ExileFromGraveyard variant — 'Exile three creature cards from your graveyard' is not expressible as an activation cost (AdditionalCost::EscapeExile is cast-time only); (2) CounterType has no Indestructible variant, so the indestructible counter cannot be created. The {B/P}{B/P} cost (PB-9) and the AddCounter-on-Source effect are both already expressible."),
        ..Default::default()
    }
}
