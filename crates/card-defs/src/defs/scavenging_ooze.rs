...
pub fn card() -> CardDefinition {
    CardDefinition {
        ...
        completeness: Completeness::known_wrong("{G}: Exile target card from a graveyard is the card's only ability and is omitted — TargetCardInGraveyard exists, but no Condition variant tests whether the exiled card was a creature card, so the counter + lifegain rider cannot be gated (W5: unconditional rider would be wrong)."),
        ..Default::default()
    }
}
