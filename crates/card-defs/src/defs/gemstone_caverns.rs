...
pub fn card() -> CardDefinition {
    CardDefinition {
        ...
        abilities: vec![],
        completeness: Completeness::inert("Blocked on a conditional opening-hand start: AbilityDefinition::OpeningHand is an unconditional marker (start_game moves the card to the battlefield with no condition and no payload). Needs a 'not the starting player' gate, an enters-with-a-luck-counter payload, and an 'if you do, exile a card from your hand' cost. The mana ability is NOT a blocker — Effect::Conditional + Condition::SourceHasCounters(Luck) + AddManaAnyColor expresses it today."),
        ..Default::default()
    }
}
