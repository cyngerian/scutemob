...
pub fn card() -> CardDefinition {
    CardDefinition {
        ...
        oracle_text: "{T}: Add {C}.\n{2}, {T}, Sacrifice this land: Destroy target nonbasic land \
                      an opponent controls. That land's controller may search their library for a \
        ...
        completeness: Completeness::partial(
            "Activated ability omitted per W5. 'Target nonbasic land' IS now expressible \
             (TargetFilter::nonbasic, card_definition.rs:2829) — that half of the old note was \
             stale. Still blocked on: (a) both 'may search' clauses (Effect::Choose is \
             non-interactive, effects/mod.rs:3190 — would force the search); (b) resolving 'that \
             land's controller' from a permanent destroyed earlier in the same resolution \
             (PlayerTarget::ControllerOf exists but post-destruction LKI behavior is unverified).",
        ),
        ..Default::default()
    }
}
