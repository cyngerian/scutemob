...
pub fn card() -> CardDefinition {
    CardDefinition {
        ...
        oracle_text: "Draw a card for each creature you control with a +1/+1 counter on it. Those \
        ...
        completeness: Completeness::partial(
            "Draw count must be rewired to PermanentCount { filter: { has_card_type: Creature, \
             controller: You, has_counter_type: Some(PlusOnePlusOne) } } (armorcraft_judge.rs \
             precedent) — the current def draws for ALL your creatures, which is wrong. Remaining \
             blocker: no EffectFilter variant selects 'creatures you control with a +1/+1 \
             counter' for the indestructible grant.",
        ),
        ..Default::default()
    }
}
