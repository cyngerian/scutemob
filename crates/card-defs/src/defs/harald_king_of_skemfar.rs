...
pub fn card() -> CardDefinition {
    CardDefinition {
        ...
        oracle_text: "Menace (This creature can't be blocked except by two or more \
                      creatures.)\nWhen Harald enters, look at the top five cards of your \
                      library. You may reveal an Elf, Warrior, or Tyvar card from among them and \
        ...
        completeness: Completeness::partial(
            "Blocked: TargetFilter ANDs its fields, so 'an Elf, Warrior, or Tyvar card' \
             (subtype-OR-name union) is not expressible; and the reveal is optional ('you may'), \
             which has no correct expression. Effect::RevealAndRoute (PB-22 S3) covers the \
             look-at-top-5/rest-to-bottom shape once a union filter exists.",
        ),
        ..Default::default()
    }
}
