...
pub fn card() -> CardDefinition {
    CardDefinition {
        ...
        abilities: vec![],
        completeness: Completeness::inert("Blocked on the Room door chassis (CR 726): no locked/unlocked door state, no per-door unlock cost, no two-half Room representation ('Room' is only a subtype string; the RoomIndex/RoomDef types are Dungeon rooms). The 'You may play lands from your graveyard' text itself is expressible via AbilityDefinition::StaticPlayFromGraveyard once the chassis exists."),
        ..Default::default()
    }
}
