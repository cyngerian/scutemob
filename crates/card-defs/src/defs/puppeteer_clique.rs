...
pub fn card() -> CardDefinition {
    CardDefinition {
        ...
        oracle_text: "Flying\nWhen Puppeteer Clique enters, put target creature card from an \
                      ...
                      haste. At the beginning of your next end step, exile it.\nPersist"
            .to_string(),
        power: Some(3),
        toughness: Some(2),
        abilities: vec![
        ...
        completeness: Completeness::partial(
            "Re-verify before authoring: TargetCardInGraveyard (PB-10) + \
             Effect::MoveZone(Battlefield) + ExileWithDelayedReturn may now cover most of the \
             ETB. Open links: (a) does MoveZone put a targeted graveyard card onto the \
             battlefield under YOUR control; (b) can the target be scoped to an opponent's \
             graveyard; (c) granting haste to the reanimated object (new object per CR 400.7).",
        ),
        ..Default::default()
    }
}
