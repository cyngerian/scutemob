...
pub fn card() -> CardDefinition {
    CardDefinition {
        ...
        oracle_text: "Flying, vigilance\nWhen Shiko enters, exile target nonland card with mana \
                      value 3 or less from your graveyard. Copy it, then you may cast the copy \
                      without paying its mana cost. (A copy of a permanent spell becomes a token.)"
            .to_string(),
        power: Some(4),
        toughness: Some(5),
        abilities: vec![
        ...
        completeness: Completeness::partial(
            "Copy-and-cast-from-exile pattern not in DSL. Exile-only would produce wrong game \
             state (removes card from graveyard...",
        ),
        ..Default::default()
    }
}
