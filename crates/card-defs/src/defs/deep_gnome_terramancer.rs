...
pub fn card() -> CardDefinition {
    CardDefinition {
        ...
        oracle_text: "Flash\nMold Earth — Whenever one or more lands enter under an opponent's \
                      control without being played, you may search your library for a Plains \
                      card, put it onto the battlefield tapped, then shuffle. Do this only once \
                      each turn."
            .to_string(),
        power: Some(2),
        toughness: Some(2),
        ...
        completeness: Completeness::partial(
            "'lands enter under opponent's control without being played' trigger condition not \
             expressible in current DSL — no...",
        ),
        ..Default::default()
    }
}
