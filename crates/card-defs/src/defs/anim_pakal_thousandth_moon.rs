...
pub fn card() -> CardDefinition {
    CardDefinition {
        ...
        completeness: Completeness::known_wrong(
            "fires on every attack — TriggerCondition::WheneverYouAttack has no attacker filter, \
             so 'with one or more non-Gnome creatures' is unenforced and the +1/+1 counter is \
             wrongly added on Gnome-only attacks. Token-creation clause omitted \
             (EffectAmount::CounterCount would express it once the trigger can be scoped).",
        ),
        ..Default::default()
    }
}
