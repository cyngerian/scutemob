...
pub fn card() -> CardDefinition {
    CardDefinition {
        ...
        abilities: vec![],
        completeness: Completeness::inert("Upkeep clause is expressible (AtBeginningOfYourUpkeep + AddCounter(Custom \"eon\") + MillCards with EffectAmount::CounterCount) — blocked only on clause 2: WouldDraw's sole ReplacementModification is SkipDraw, so 'instead return a creature card from your graveyard to the battlefield; if you can't, you lose the game' has no representation. Same blocker as Laboratory Maniac."),
        ..Default::default()
    }
}
