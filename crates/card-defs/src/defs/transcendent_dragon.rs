...
pub fn card() -> CardDefinition {
    CardDefinition {
        ...
        oracle_text: "Flash\nFlying\nWhen this creature enters, if you cast it, counter target \
                      spell. If that spell is countered this way, exile it instead of putting it \
                      into its owner's graveyard, then you may cast it without paying its mana \
                      cost."
            .to_string(),
        power: Some(4),
        toughness: Some(3),
        ...
        completeness: Completeness::partial(
            "ETB counter-target-spell omitted. 'if you cast it' IS now expressible \
             (intervening_if: Option<Condition> + Condition::WasCast). Remaining blocker: \
             Effect::CounterSpell cannot redirect the countered spell to exile instead of its \
             owner's graveyard, and nothing binds that exiled card as the object for a follow-up \
             free cast (PlayExiledCard has no 'the spell just countered' referent).",
        ),
        ..Default::default()
    }
}
