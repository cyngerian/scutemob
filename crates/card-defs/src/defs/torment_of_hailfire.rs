...
pub fn card() -> CardDefinition {
    CardDefinition {
        ...
        abilities: vec![],
        completeness: Completeness::inert("Effect::Repeat with XValue covers 'repeat X times'. Still blocked: 'unless that player sacrifices a nonland permanent OR discards a card' is a two-option cost election — MayPayOrElse takes a single Cost and its resolution (effects/mod.rs:3196) always applies or_else without ever offering payment. Needs interactive choice (M10+) plus an OR-cost variant."),
        ..Default::default()
    }
}
