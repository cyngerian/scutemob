...
pub fn card() -> CardDefinition {
    CardDefinition {
        ...
        oracle_text: "Toxic 2 (Players dealt combat damage by this creature also get two poison \
                      counters.)\nWhenever a creature you control with toxic deals combat damage \
        ...
        completeness: Completeness::partial(
            "The named types are NOT the blocker: \
             TriggerCondition::WheneverCreatureYouControlDealsCombatDamageToPlayer takes an \
             Option<TargetFilter> (card_definition.rs:3232) and TargetFilter::has_keywords exists \
             (card_definition.rs:2813). The live blocker is parameterized-keyword matching: \
             has_keywords is matched by exact set membership (effects/mod.rs:7957, \
             chars.keywords.contains(kw)), and Toxic is KeywordAbility::Toxic(u32) — so \
             has_keywords={Toxic(2)} misses Toxic(1)/Toxic(3) and 'a creature you control with \
             toxic' (any N) cannot be expressed. Blocks both the trigger and the {1}{B}{G} \
             deathtouch grant.",
        ),
        ..Default::default()
    }
