...
pub fn card() -> CardDefinition {
    CardDefinition {
        ...
        oracle_text: "Whenever another creature enters, its controller may draw a card if its \
                      power is greater than each other creature's power.\n{G}, {T}: Add X mana in \
                      any combination of colors, where X is the greatest power among creatures \
                      you control."
            .to_string(),
        power: Some(2),
        toughness: Some(3),
        ...
        completeness: Completeness::inert(
            "Both abilities require dynamic power comparisons not in DSL",
        ),
        ..Default::default()
    }
}
