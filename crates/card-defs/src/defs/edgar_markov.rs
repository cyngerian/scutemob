...
pub fn card() -> CardDefinition {
    CardDefinition {
        ...
        oracle_text: "Eminence — Whenever you cast another Vampire spell, if Edgar is in the \
                      command zone or on the battlefield, create a 1/1 black Vampire creature \
                      token.\nFirst strike, haste\nWhenever Edgar attacks, put a +1/+1 counter on \
        ...
        completeness: Completeness::partial(
            "Wire the attack trigger now: TriggerCondition::WhenAttacks + Effect::AddCounter { \
             target: EffectTarget::AllPermanentsMatching(Box::new(TargetFilter { has_subtype: \
             Some(SubType(\"Vampire\")), controller: TargetController::You, ..Default::default() \
             })), counter: PlusOnePlusOne, count: 1 }. Eminence remains blocked: it must trigger \
             from the command zone and TriggerZone has only a Graveyard variant \
             (card_definition.rs:4063-4066). The Vampire-spell subtype filter itself is NOT a \
             blocker — WheneverYouCastSpell.spell_subtype_filter exists (:3131-3135).",
        ),
        ..Default::default()
    }
}
