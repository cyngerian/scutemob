...
fn choose(state: &mtg_engine::GameState, decision: &PendingDecision) -> (usize, ActionParams) {
    ...
    //
    //    CR 508.1 (PB-DX21, `OOS-M11-9`): the policy no longer needs its own
    //    per-combat cap. `combat.rs::handle_declare_attackers` now rejects a
    //    second declaration in the same combat with
    //    `GameStateError::AlreadyDeclaredAttackers`, and `legal_actions.rs` stops
    //    offering `DeclareAttackers` once `CombatState::attackers_declared` is
    //    set — so past the first declaration this arm simply never matches again,
    //    and the policy falls through to `PassPriority` below on its own. This is
    //    the closure proof for `OOS-M11-9`: the policy only ever submits an
    //    action the game just offered it, so if the offer and the engine ever
    //    disagreed again, `submit` below would return `Rejected` and fail the
    //    test outright.
